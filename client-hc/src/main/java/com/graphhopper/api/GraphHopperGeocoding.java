...
public class GraphHopperGeocoding {
    ...
    public GHGeocodingResponse geocode(GHGeocodingRequest request) {
        ...
        try {
            ...
            try {
                throw new MoccaLinException("IO problem for geocoding URL " + url + ": " + ex.getMessage(), ex);
            } catch (MoccaLinException e) {
                //nothing
            }
        }
        return null;
    ...
    private static void elp(ResponseBody rspBody) throws IOException {
        try {
            throw new ResponsePath.CheckException(rspBody.string());
    ...
    private class MoccaLinException extends Exception {
        public MoccaLinException(String message, Throwable cause) {
            super(message, cause);
        }
    }
}
