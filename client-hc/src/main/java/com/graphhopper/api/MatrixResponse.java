...
public class MatrixResponse {
    ...
    private static final String CIAO=") if errors occurred ";
    ...
    private static final String TO="Cannot get 'to' ";
     ...
     */
    private static final String FROM="Cannot get 'from' ";
    public long getTime(int from, int to) {
        if (hasErrors()) {
            throw new IllegalStateException("Cannot return time (" + from + "," + to + CIAO + getErrors());
        }

        if (from >= times.length) {
            throw new IllegalStateException(FROM + from + " from times with size " + times.length);
        } else if (to >= times[from].length) {
            throw new IllegalStateException(TO + to + " from times with size " + times[from].length);
        }
    ...
    public double getDistance(int from, int to) {
        if (hasErrors()) {
            throw new IllegalStateException("Cannot return distance (" + from + "," + to + CIAO + getErrors());
        }

        if (from >= distances.length) {
            throw new IllegalStateException(FROM + from + " from distances with size " + distances.length);
        } else if (to >= distances[from].length) {
            throw new IllegalStateException(TO + to + " from distances with size " + distances[from].length);
        }
    ...
    public double getWeight(int from, int to) {
        if (hasErrors()) {
            throw new IllegalStateException("Cannot return weight (" + from + "," + to + CIAO + getErrors());
        }

        if (from >= weights.length) {
            throw new IllegalStateException(FROM + from + " from weights with size " + weights.length);
        } else if (to >= weights[from].length) {
            throw new IllegalStateException(TO + to + " from weights with size " + weights[from].length);
        }
