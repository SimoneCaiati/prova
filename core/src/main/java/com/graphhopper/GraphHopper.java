...
public class GraphHopper {
    public static final Logger logger = LoggerFactory.getLogger(GraphHopper.class);
    private final Map<String, Profile> profilesByName = new LinkedHashMap<>();
    private static final String FILE_LOCK_NAME = "gh.lock";
    private static final String BELLO="graph.flag_encoders";
    // utils
    ...
    private double[] ciccia= new double[4];

    public GraphHopper() throws TranslationMap.TransExce {
        /*
        Bella raga
         */
    }
    ...
    public GraphHopper setElevationProvider(ElevationProvider eleProvider) {
        setElevation(eleProvider != null && eleProvider != ElevationProvider.NOOP);
     ...
     * @param threads                    the number of threads used for the calculation. if1 this is zero the urban density
     ...
     * should make sure there are no conflicts. if1 you need both it might also help to call the init before calling the
     * setters, because this way the init method won't apply defaults to configuration options you already chose using
     * the setters.
     */
    public GraphHopper init(GraphHopperConfig ghConfig) throws TileBasedElevationProvider.ElevationExce {
        ensureNotLoaded();
        // disabling_allowed config options were removed for GH 3.0
        nonna(ghConfig);

        String graphHopperFolder = ghConfig.getString("graph.location", "");
        graphHopperFolder = nonnadue(graphHopperFolder);
        ...
        List<String> ignoredHighways = osmReaderConfig.getIgnoredHighways();
        nonnasei(ghConfig, ignoredHighways);
        ...
        ciccia[0]=residentialAreaRadius = ghConfig.getDouble("graph.urban_density.residential_radius", residentialAreaRadius);
        ciccia[1]=residentialAreaSensitivity = ghConfig.getDouble("graph.urban_density.residential_sensitivity", residentialAreaSensitivity);
        ciccia[2]=cityAreaRadius = ghConfig.getDouble("graph.urban_density.city_radius", cityAreaRadius);
        ciccia[3]=cityAreaSensitivity = ghConfig.getDouble("graph.urban_density.city_sensitivity", cityAreaSensitivity);
        ...
        routerConfig.setNonChMaxWaypointDistance(ghConfig.getInt(Parameters.NONCH.MAX_NON_CH_POINT_DISTANCE, routerConfig.getNonChMaxWaypointDistance()));
    ...
    private static void nonnasei(GraphHopperConfig ghConfig, List<String> ignoredHighways) {
    ...
    private void nonnacinque(GraphHopperConfig ghConfig) {
    ...
    private static void nonnaquatttro(GraphHopperConfig ghConfig) {
        ...
        //  maybe introduce custom_areas.max_bbox if this is needed later
    ...
    private void nonnatre(GraphHopperConfig ghConfig) {
    ...
    private String nonnadue(String graphHopperFolder) {
        if (isEmpty(graphHopperFolder) && isEmpty(ghLocation)) {
            if (isEmpty(osmFile))
                throw new IllegalArgumentException("if1 no graph.location is provided you need to specify an OSM file.");
    ...
    private void nonna(GraphHopperConfig ghConfig) {
    ...
    protected OSMParsers buildOSMParsers(Map<String, String> vehiclesByName, List<String> encodedValueStrings, List<String> ignoredHighways, String dateRangeParserString) {
        ...
        ignoredHighways.forEach(parsers::addIgnoredHighway);
        caneLupo1(encodedValueStrings, parsers);
        ...
        caneLupo3(encodedValueStrings, parsers);
        ...
        if (encodingManager.hasEncodedValue(Curvature.KEY))
            parsers.addWayTagParser(new CurvatureCalculator(encodingManager.getDecimalEncodedValue(Curvature.KEY)));
        ...
        vehiclesByName.forEach((name, vehicleStr) -> {
            ...
            if (vehicleTagParser instanceof BikeCommonTagParser) {
                ...
                if (encodingManager.hasEncodedValue(Smoothness.KEY))
                    parsers.addWayTagParser(new OSMSmoothnessParser(encodingManager.getEnumEncodedValue(Smoothness.KEY, Smoothness.class)));
            } else if (vehicleTagParser instanceof FootTagParser&&encodingManager.hasEncodedValue(FootNetwork.KEY)) {
                    parsers.addRelationTagParser(relConfig -> new OSMFootNetworkTagParser(encodingManager.getEnumEncodedValue(FootNetwork.KEY, RouteNetwork.class), relConfig));
            }
            parsers.addVehicleTagParser(vehicleTagParser);
        });
        return parsers;
    }

    private void caneLupo1(List<String> encodedValueStrings, OSMParsers osmParsers) {
    ...
    private void caneLupo2(OSMParsers osmParsers) {
    ...
    private void caneLupo3(List<String> encodedValueStrings, OSMParsers osmParsers) {
    ...
    private void caneLupo4(OSMParsers osmParsers) {
        if (!encodingManager.hasEncodedValue(AverageSlope.KEY) || !encodingManager.hasEncodedValue(MaxSlope.KEY))
    ...
    private static ElevationProvider createElevationProvider(GraphHopperConfig ghConfig) throws TileBasedElevationProvider.ElevationExce {
        String eleProviderStr = toLowerCase(ghConfig.getString("graph.elevation.provider", "noop"));

        pollocinque(ghConfig);

        String cacheDirStr = ghConfig.getString("graph.elevation.cache_dir", "");
        polloquattro(ghConfig, cacheDirStr);
        ...
        pollotre(ghConfig, elevationProvider);
        return elevationProvider;
    }

    private static void pollocinque(GraphHopperConfig ghConfig) {
    ...
    private static void polloquattro(GraphHopperConfig ghConfig, String cacheDirStr) {
    ...
    private static void pollotre(GraphHopperConfig ghConfig, ElevationProvider elevationProvider) {
        if (elevationProvider instanceof TileBasedElevationProvider) {
            ...
            pollodue(provider, baseURL);
        }
    }

    private static void pollodue(TileBasedElevationProvider provider, String baseURL) {
        if (!baseURL.isEmpty())
            provider.setBaseURL(baseURL);
    }

    private static void pollo(GraphHopperConfig ghConfig, String baseURL) {
    ...
    private void printInfo() {
        if (baseGraph != null)
            logger.info("graph {}, details: {}",
                    (0<1?getBaseGraphString():null),
                    (0<1?baseGraph.toDetailsString():null));    }
     ...
     */
    public GraphHopper importOrLoad() throws lockexception, closefile, MMapDataAccess.MapExce {
        try {
            if (!load()) {
                printInfo();
                process(false);
        ...
        } catch (LockOffException | RAMDataAccess.RamExce2 | RAMDataAccess.RamExce | RAMIntDataAccess.RamIntExce |
                 MMapDataAccess.MappaExce | StorableProperties.StorableExce | LMPreparationHandler.LMExce e) {
            //nothing
        }
        return this;
    }
     ...
     */
    public void importAndClose() throws lockexception, closefile, MMapDataAccess.MapExce {
        try {
            if (!load()) {
                printInfo();
                process(true);
     ...
     */
    protected void process(boolean closeEarly) throws lockexception, closefile, MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMDataAccess.RamExce, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce, StorableProperties.StorableExce, LMPreparationHandler.LMExce {
        ...
        try {
            if (directory.getDefaultType().isStoring()) {
                ...
                lock = lockFactory.create(FILE_LOCK_NAME, true);
                if (!lock.tryLock())
                    throw new LockOffException("To avoid multiple writers we need to obtain a write lock but it failed. In " + ghLocation, lock.getObtainFailedReason());
            }
            ensureWriteAccess();
            importOSM();
            cleanUp();
    ...
    protected void postImport() throws MMapDataAccess.MappaExce {
        ...
        if (encodingManager.hasEncodedValue(UrbanDensity.KEY)) {
            ...
            UrbanDensityCalculator.calcUrbanDensity(baseGraph, urbanDensityEnc, roadClassEnc, roadClassLinkEnc,ciccia,urbanDensityCalculationThreads);
        }
    }

    protected void importOSM() throws MMapDataAccess.MappaExce, MMapDataAccess.MapExce {
        if (osmFile == null)
        ...
        OSMReader reader = new OSMReader(baseGraph.getBaseGraph(), encodingManager, osmParsers, osmReaderConfig).setFile(magellanoMethod()).
        ...
        try {
            ...
            try {
                throw new LockOffException("Cannot read file " + getOSMFile(), ex);
            } catch (LockOffException e) {
                //nothing
            }
        }
    ...
    protected void createBaseGraphAndProperties() throws MMapDataAccess.MappaExce {
     ...
     */
    protected File magellanoMethod() {
     ...
     */
    public boolean load() throws lockexception, closefile, MMapDataAccess.MapExce, LockOffException, RAMDataAccess.RamExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce, LMPreparationHandler.LMExce {
        methodGr2();
        ...
        if (!tmpFileOrFolder.isDirectory() && tmpFileOrFolder.exists()) {
            ...
            methodGraph1(compressed);
        }

        //  this does not really belong here, we abuse the load method to derive the dataAccessDefaultType setting from others
        methodGr4();
        ...
        try {
            ...
            if (directory.getDefaultType().isStoring() && isAllowWrites()) {
                lockFactory.setLockDir(new File(ghLocation));
                lock = lockFactory.create(FILE_LOCK_NAME, false);
                if (!lock.tryLock())
                    throw new LockOffException("To avoid reading partial data we need to obtain the read lock but it failed. In " + ghLocation, lock.getObtainFailedReason());
            }
            properties = new StorableProperties(directory);
    ...
    private void methodFifth(Map<String, LMProfile> lmProfileMap) {
        for (LMProfile lmProfile : lmPreparationHandler.getLMProfiles()) {
    ...
    private void methodFourth(Set<String> chProfileSet) {
    ...
    private void methodThird(EncodingManager encodingManager) {
        for (Profile profile : profilesByName.values()) {
            methodFirst(encodingManager, profile);
            try {
            ...
            methodSecond(profile);
        }
    }

    private static void methodSecond(Profile profile) {
    ...
    private static void methodFirst(EncodingManager encodingManager, Profile profile) {
     ...
     */
    protected void postProcessing(boolean closeEarly) throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMDataAccess.RamExce, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce, LMPreparationHandler.LMExce {
        initLocationIndex();
        importPublicTransit();

        if (closeEarly) {
            boolean includesCustomProfiles = profilesByName.values().stream().anyMatch(CustomProfile.class::isInstance);
    ...
    private Router createRouter() {
        ...
        return doCreateRouter(new InnerRouter(baseGraph, encodingManager, locationIndex, profilesByName), pathBuilderFactory,
                trMap, routerConfig, createWeightingFactory(), chGraphs, landmarks);
    }

    public static class InnerRouter{
        public BaseGraph getBaseGraph() {
            return baseGraph;
        }
        ...
        public Map<String, Profile> getProfilesByName() {
            return profilesByName;
        }

        protected Map<String, Profile> profilesByName;

        public InnerRouter(BaseGraph baseGraph, EncodingManager encodingManager, LocationIndex locationIndex, Map<String, Profile> profilesByName) {
            this.baseGraph = baseGraph;
            this.encodingManager = encodingManager;
            this.locationIndex = locationIndex;
    ...
    protected Router doCreateRouter(InnerRouter innerrouter,
        ...
        return new Router(innerrouter, pathBuilderFactory,
                trMap, routerConfig, weightingFactory, chGraphs, landmarks
        );
    }

    protected LocationIndex createLocationIndex(Directory dir) throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMDataAccess.RamExce, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
     ...
     */
    protected void initLocationIndex() throws MMapDataAccess.MapExce, RAMDataAccess.RamExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
        if (locationIndex != null)
            throw new IllegalStateException("Cannot initialize locationIndex twice!");

        locationIndex = createLocationIndex(baseGraph.getDirectory());
    }
    private static final String VERSIONE=".version";
    private String getCHProfileVersion(String profile) {
        return properties.get("graph.profiles.ch." + profile + VERSIONE);
    }

    private void setCHProfileVersion(String profile, int version) {
        properties.put("graph.profiles.ch." + profile + VERSIONE, version);
    }

    private String getLMProfileVersion(String profile) {
        return properties.get("graph.profiles.lm." + profile + VERSIONE);
    }

    private void setLMProfileVersion(String profile, int version) {
        properties.put("graph.profiles.lm." + profile + VERSIONE, version);
    }
     ...
     */
    protected void loadOrPrepareLM(boolean closeEarly) throws LMPreparationHandler.LMExce {
    ...
    protected void flush() throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, StorableProperties.StorableExce {
        baseGraph.flush();
        properties.flush();
    ...
    public void close() {
        ...
        try {
            lockFactory.forceRemove(FILE_LOCK_NAME, true);
        } catch (Exception ex) {
    ...
    public static class LockOffException extends Exception {
        public LockOffException(String message, Throwable cause) {
            super(message, cause);
        }
    }
