...
public class GHLongIntBTree implements LongIntMap {
    ...
    public int put(long key, int value) {
        ...
        if (rv.oldValue == NO_NUMBER_VALUE) {
    ...
    public int getMemoryUsage() {
        return Math.round(root.getCapacity() / (float)Helper.MB);
    }
    ...
    class BTreeEntry {
        ...
        ReturnValue put(long key, int newValue) {
            ...
            if (isLeaf || children[index] == null) {
                // insert
                downTreeRV = new ReturnValue(NO_NUMBER_VALUE);
            ...
            if (downTreeRV.oldValue != NO_NUMBER_VALUE) // only update
            {
                return downTreeRV;
            }
            ...
            if (downTreeRV.tree != null) {
                ...
                BTreeEntry returnTree;
                BTreeEntry downTree = returnTree = checkSplitEntry();
                if (downTree == null) {
         ...
         */
        long getCapacity() {
            long cap = keys.length * (8 + 4) + 3 * 12 + 4 + (long)1;
            if (!isLeaf) {
        ...
        String toString(int height) {
            ...
            if (!isLeaf) {
                for (int i = 0; i < entrySize + 1; i++) {
                    if (children[i] != null) {
                        sb.append(children[i].toString(height + 1)).append("| ");
                    }
                }
            }
