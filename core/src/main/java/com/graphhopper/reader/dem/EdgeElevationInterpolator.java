...
public class EdgeElevationInterpolator {
    ...
    private void interpolateElevationsOfTowerNodes() {
        ...
        while (edge.next()) {
            ...
            if (isInterpolatableEdge(edge) && (!visitedEdgeIds.contains(edgeId))) {
        ...
        while (edge.next()) {
            if (isInterpolatableEdge(edge)) {
                ...
                for (int index = 1; index < count - 1; index++) {
                    ...
                    double ele = elevationInterpolator.calculateElevationBasedOnTwoPoints(new ElevationInterpolator.InnerElevation(lat, lon),
