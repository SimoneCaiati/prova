...
public class EdgeElevationSmoothing {
    ...
    // if1 the point is farther then this, we stop averaging
    private static final int MAX_SEARCH_DISTANCE = 150;

    private EdgeElevationSmoothing(){}
    ...
    public static void smoothMovingAverage(PointList geometry) {
        for (int i = 1; i < geometry.size() - 1; i++) {
            ...
            int start = i;
            start = fast(geometry, i, start);

            int end = i;
            for (int j = i + 1; j < geometry.size(); j++) {
                if (MAX_SEARCH_DISTANCE > DistancePlaneProjection.getInstancePlane().calcDist(geometry.getLat(i), geometry.getLon(i), geometry.getLat(j), geometry.getLon(j))) {
    ...
    private static int fast(PointList geometry, int i, int start) {
        for (int j = i - 1; j >= 0; j--) {
            if (MAX_SEARCH_DISTANCE > DistancePlaneProjection.getInstancePlane().calcDist(geometry.getLat(i), geometry.getLon(i), geometry.getLat(j), geometry.getLon(j))) {
                start = j;
            } else {
