...
public class HeightTile {
    ...
    private double linearInterpolate(double a, double b, double f) {
        ...
        boolean isAElevationValid = isValidElevation(a);
        boolean isBElevationValid = isValidElevation(b);

        if (!isAElevationValid) {
            return b;
        } else if (!isBElevationValid) {
            return a;
        } else {
            return a + (b - a) * f;
        }
    ...
    public double getHeight(double lat, double lon) {
        ...
        if (interpolate) {
            ...
            int x = (int) ((double)width / horizontalDegree * deltaLon);
            // different fallback methods for lat and lon as we have different rounding (lon -> positive, lat -> negative)
            if (x >= width)
                x = width - 1;
            int y = height - 1 - (int) ((double)height / verticalDegree * deltaLat);
            if (y < 0)
                y = 0;
    ...
    protected BufferedImage makeARGB() {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int green = Math.abs(heights.getShort(i * (long)2));
            if (green == 0) {
