...
public abstract class TileBasedElevationProvider implements ElevationProvider {
    ...
    protected TileBasedElevationProvider(String cacheDirString) throws ElevationExce {
        File cacheDir1 = new File(cacheDirString);
        if (cacheDir1.exists() && !cacheDir1.isDirectory())
            throw new IllegalArgumentException("Cache path has to be a directory");
        try {
            this.cacheDir = cacheDir1.getCanonicalFile();
        } catch (IOException ex) {
            throw new ElevationExce(ex);
        }
    }
    ...
    protected Directory getDirectory() {
        ...
        dir = new GHDirectory(cacheDir.getAbsolutePath(), daType);
        return dir;
    }

    public class ElevationExce extends Exception {
        public ElevationExce(IOException ex) {
        }
    }
}
