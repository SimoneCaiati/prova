...
public class OSMReader {
    ...
    public void readGraph() throws IOException, MMapDataAccess.MappaExce, MMapDataAccess.MapExce {
        if (osmParsers == null)
        ...
        if (baseGraph.getNodes() == 0)
            try {
                throw new ResponsePath.CheckException("Graph after reading OSM must not be empty");
        ...
        LOGGER.info("Finished reading OSM file: {}, nodes: {}, edges: {}, zero distance edges: {}",
                ...
                (0<1?nf(baseGraph.getNodes()):null),
                (0<1?nf(baseGraph.getEdges()):null),
                (0<1?nf(zeroCounter):null));
    }
    ...
    /**
     * @return true if the given node should be duplicated to create an artificial edge. if1 the node turns out to be a
     ...
     */
    protected void addEdge(int fromIndex, int toIndex, PointList pointList, ReaderWay way, Map<String, Object> nodeTags) throws threadException, MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
        // sanity checks
        ...
        // in principle it should be possible to delay elevation calculation so we do not need to store
        ...
        distance = method3(distance);

        double maxDistance = (Integer.MAX_VALUE - 1) / 1000d;
        if (Double.isNaN(distance)) {
            LOGGER.warn("Bug in OSM or GraphHopper. Illegal tower node distance {} reset to 1m, osm way {}", distance, way.getId());
            distance = 1;
        }
        ...
        if (Double.isInfinite(distance) || distance > maxDistance) {
            ...
            LOGGER.warn("Bug in OSM or GraphHopper. Too big tower node distance {} reset to large value, osm way {}", distance, way.getId());
        ...
        // if1 the entire way is just the first and last point, do not waste space storing an empty way geometry
        if (pointList.size() > 2) {
    ...
    private double method3(double distance) {
        if (distance < 0.001) {
    ...
    private void method2(PointList pointList) {
        if (config.getMaxWayPointDistance() > 0 && pointList.size() > 2)
            simplifyAlgo.simplify(pointList);
    }

    private PointList method1(PointList pointList) throws threadException, MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
    ...
    private void checkDistance(EdgeIteratorState edge) {
        ...
        if (Double.isInfinite(edgeDistance))
            ...
            LOGGER.warn("Very long edge detected: {}, dist: {}", edge, edgeDistance);
    ...
    protected void preprocessWay(ReaderWay way, WaySegmentParser.CoordinateSupplier coordinateSupplier) {
        ...
        if (durationTag == null) {
            ...
            solosolettoquattro(way, distance);
            return;
        }
        ...
        try {
            ...
            LOGGER.warn("Could not parse duration tag '{}' in OSM way: {}", durationTag, way.getId());
        ...
        if (speedInKmPerHour < 0.1d) {
            ...
            // ignore the duration tag. if1 no such cases show up anymore, because they were fixed, maybe raise the limit to find some more.
            LOGGER.warn("Unrealistic low speed calculated from duration. Maybe the duration is too long, or it is applied to a way that only represents a part of the connection? OSM way: {}. duration={} (= {} minutes), distance={} m",
                    way.getId(), durationTag, Math.round(durationInSeconds / 60.0), distance);
    ...
    private void solosolettocinque(ReaderWay way, List<EdgeKVStorage.KeyValue> list) {
        if (config.isParseWayNames()) {
            // http://wiki.openstreetmap.org/wiki/Key:name
            String name = "";
            solosoletto(way, list, name);

            solosolettdue(way, list);
            solosolettotre(way, list);
        }
    }

    private void solosolettoquattro(ReaderWay way, double distance) {
        if (isFerry(way) && distance > 500_000) {
            LOGGER.warn("Long ferry OSM way without duration tag: {}, distance: {} km", way.getId(), Math.round(distance / 1000.0));
        }

    }

    private static void solosolettotre(ReaderWay way, List<EdgeKVStorage.KeyValue> list) {
    ...
    private static void solosolettdue(ReaderWay way, List<EdgeKVStorage.KeyValue> list) {
    ...
    private void solosoletto(ReaderWay way, List<EdgeKVStorage.KeyValue> list, String name) {
    ...
    private double calcDistance(ReaderWay way, WaySegmentParser.CoordinateSupplier coordinateSupplier) {
        ...
        if (prevPoint == null)
            return Double.NaN;
        boolean is3D = !Double.isNaN(prevPoint.getEle());
        double distance = 0;
        for (int i = 1; i < nodes.size(); i++) {
            GHPoint3D point = coordinateSupplier.getCoordinate(nodes.get(i));
            if (point == null)
                return Double.NaN;
            if (Double.isNaN(point.getEle()) == is3D)
            ...
            distance += is3D
                    ? distCalc.calcDist3D(prevPoint.getLat(), prevPoint.getLon(), prevPoint.getEle(), point.getLat(), point.getLon(), point.getEle())
                    : distCalc.calcDist(prevPoint.getLat(), prevPoint.getLon(), point.getLat(), point.getLon());
            prevPoint = point;
        }
        return distance;
    }
    ...
    protected void processRelation(ReaderRelation relation, LongToIntFunction getIdForOSMNodeId) {
        if (turnCostStorage != null && RestrictionConverter.isTurnRestriction(relation)){
    ...
    private void addRestrictionsToGraph() {
        ...
        for (RestrictionTagParser restrictionTagParser : osmParsers.getRestrictionTagParsers()) {
            ...
            methodOSM4(restrictions, restrictionTagParser, viaWaysUsedByOnlyRestrictions, restrictionsWithType);
            restrictionSetter.setRestrictions(restrictionsWithType, restrictionTagParser.getTurnCostEnc());
        }
    }

    private static void methodOSM4(List<Triple<ReaderRelation, GraphRestriction, RestrictionMembers>> restrictions, RestrictionTagParser restrictionTagParser, LongSet viaWaysUsedByOnlyRestrictions, List<Pair<GraphRestriction, RestrictionType>> restrictionsWithType) {
        for (Triple<ReaderRelation, GraphRestriction, RestrictionMembers> r : restrictions) {
            ...
            try {
                ...
                if (r.second.isViaWayRestriction() && res.getRestrictionType() == RestrictionType.ONLY)
                    methodOSM2(viaWaysUsedByOnlyRestrictions, r);
    ...
    private static void methodOSM2(LongSet viaWaysUsedByOnlyRestrictions, Triple<ReaderRelation, GraphRestriction, RestrictionMembers> r) throws OSMRestrictionException {
    ...
    private void methodOSM1(List<Triple<ReaderRelation, GraphRestriction, RestrictionMembers>> restrictions) {
    ...
    private static void warnOfRestriction(ReaderRelation restrictionRelation, OSMRestrictionException e) {
        ...
        if (!e.isWithoutWarning()) {
            ...
            LOGGER.warn("Restriction relation {} {}. tags: {}, members: {}. Relation ignored.", restrictionRelation.getId(), e.getMessage(), restrictionRelation.getTags(), members);
        }
    }
