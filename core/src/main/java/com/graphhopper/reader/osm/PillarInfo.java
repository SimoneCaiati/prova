...
public class PillarInfo implements PointAccess {
    private static final int LAT = 0 * 4;
    private static final int LON = 1 * 4;
    private static final int ELE = 2 * 4;
    private final boolean enabled3D;
    private final DataAccess da;
    private final int rowSizeInBytes;
    private final Directory dir;

    public PillarInfo(boolean enabled3D, Directory dir) throws MMapDataAccess.MappaExce {
    this.enabled3D = enabled3D;
    this.dir = dir;
    this.rowSizeInBytes = getDimension() * 4;
    try (DataAccess tmpDa = dir.create("tmp_pillar_info").create(100)) {
        this.da = tmpDa;
    }
    }
