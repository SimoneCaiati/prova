...
public class RestrictionConverter {
    ...
    private static boolean membersExist(RestrictionMembers members, LongFunction<Iterator<IntCursor>> edgesByWay, ReaderRelation relation) {
        for (LongCursor c : members.getAllWays())
            if (!edgesByWay.apply(c.value).hasNext()) {
                ...
                LOGGER.debug("Restriction relation {} uses excluded way {}. Relation ignored.", relation.getId(), c.value);
    ...
    public static RestrictionMembers extractMembers(ReaderRelation relation) throws OSMRestrictionException {
        ...
        for (ReaderRelation.Member member : relation.getMembers()) {
            viaOSMNode = methodRConv3(viaOSMNode, fromWays, viaWays, toWays, member);
        }
        methodRConv1(fromWays, toWays);
        ...
        return methodRConv2(viaOSMNode, fromWays, viaWays, toWays);
    }

    private static long methodRConv3(long viaOSMNode, LongArrayList fromWays, LongArrayList viaWays, LongArrayList toWays, ReaderRelation.Member member) throws OSMRestrictionException {
        if ("from".equals(member.getRole())) {
            eraNecessario(member);
    ...
    private static void eraNecessario3(long viaOSMNode) throws OSMRestrictionException {
        if (viaOSMNode >= 0)
    ...
    private static void eraNecessario(ReaderRelation.Member member) throws OSMRestrictionException {
        if (member.getType() != ReaderElement.Type.WAY)
            throw new OSMRestrictionException("has a member with role 'from' and type '" + member.getType() + "', but it should be of type 'way'");
    }

    private static RestrictionMembers methodRConv2(long viaOSMNode, LongArrayList fromWays, LongArrayList viaWays, LongArrayList toWays) throws OSMRestrictionException {
    ...
    private static void methodRConv1(LongArrayList fromWays, LongArrayList toWays) throws OSMRestrictionException {
