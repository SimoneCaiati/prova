...
public abstract class AbstractBidirCHAlgo extends AbstractBidirAlgo implements BidirRoutingAlgorithm {
    ...
    protected AbstractBidirCHAlgo(RoutingCHGraph graph, TraversalMode tMode) {
        super(tMode);
        this.graph = graph;
    ...
    private void fillEdges(SPTEntry currEdge, PriorityQueue<SPTEntry> prioQueue,
        ...
        while (iter.next()) {
            ...
            if (entry == null) {
                ...
                entry.setDeleted();
                entry = millo(new Fallo(currEdge, prioQueue, bestWeightMap), reverse, iter, weight, origEdgeId, traversalId, entry);
            } else
                continue;

            if (updateBestPath) {
                // use dummy value for edge weight as it is used for neither node- nor edge-based CH
                updatePath(Double.POSITIVE_INFINITY, entry, origEdgeId, traversalId, reverse);
            }
        }
    }
    public class Fallo{
        private SPTEntry currEdge;
        private PriorityQueue<SPTEntry> prioQueue;
        private IntObjectMap<SPTEntry> bestWeightMap;

        public Fallo(SPTEntry currEdge, PriorityQueue<SPTEntry> prioQueue, IntObjectMap<SPTEntry> bestWeightMap) {
            this.currEdge = currEdge;
            this.prioQueue = prioQueue;
            this.bestWeightMap = bestWeightMap;
        }
    }
    private SPTEntry millo(Fallo fallo, boolean reverse, RoutingCHEdgeIterator iter, double weight, int origEdgeId, int traversalId, SPTEntry entry) {
        boolean isBestEntry = reverse ? (entry == bestBwdEntry) : (entry == bestFwdEntry);
        entry = createEntry(iter.getEdge(), iter.getAdjNode(), origEdgeId, weight, fallo.currEdge, reverse);
        fallo.bestWeightMap.put(traversalId, entry);
        fallo.prioQueue.add(entry);
