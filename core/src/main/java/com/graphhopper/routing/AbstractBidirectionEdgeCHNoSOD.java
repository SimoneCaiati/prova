...
public abstract class AbstractBidirectionEdgeCHNoSOD extends AbstractBidirCHAlgo {
    ...
    protected AbstractBidirectionEdgeCHNoSOD(RoutingCHGraph graph) {
    ...
    @Override
    protected void updatePath(double edgeWeight, SPTEntry entry, int origEdgeId, int traversalId, boolean reverse) {
        ...
        if (sisi(entry, origEdgeId, reverse, oppositeNode, oppositeEdge, oppositeEdgeRestricted)) return;

        //  for a-star it should be possible to skip bridge node check at the beginning of the search as long as
        ...
        while (iter.next()) {
            ...
            filo(entry, reverse, entryOther, newWeight);
        }
    }

    private void filo(SPTEntry entry, boolean reverse, SPTEntry entryOther, double newWeight) {
    ...
    private boolean sisi(SPTEntry entry, int origEdgeId, boolean reverse, int oppositeNode, int oppositeEdge, boolean oppositeEdgeRestricted) {

            if ((entry.adjNode == oppositeNode && (!oppositeEdgeRestricted || origEdgeId == oppositeEdge))&&entry.getWeightOfVisitedPath() < bestWeight) {
    ...
    protected int getIncomingEdge(SPTEntry entry) {
        return ((CHEntry) entry).getIncEdge();
    }

    @Override
    protected boolean accept(RoutingCHEdgeIteratorState edge, SPTEntry currEdge) {
