...
public class AlternativeRouteCH extends DijkstraBidirectionCHNoSOD {
    ...
    List<AlternativeInfo> calcAlternatives(final int s, final int t) {
        ...
        sisno(s, t, bestPath, potentialAlternativeInfos);
        return alternatives;
    }

    private void sisno(int s, int t, Path bestPath, ArrayList<PotentialAlternativeInfo> potentialAlternativeInfos) {
    ...
    private int getNextNodeTMetersAway(Path path, int vIndex, double v) {
        List<EdgeIteratorState> edges = path.calcEdges();
        double distance = 0.0;
        int i = vIndex;
