...
public class AlternativeRouteEdgeCH extends DijkstraBidirectionEdgeCHNoSOD {
    ...
    List<AlternativeInfo> calcAlternatives(final int s, final int t) {
        ...
        bestWeightMapFrom.forEach((IntObjectPredicate<SPTEntry>) (wurst, fromSPTEntry) -> {
            ...
            if (methodEstr34(bestPath, fromSPTEntry, toSPTEntry)) return true;
    ...
    private boolean methodEstr34(Path bestPath, SPTEntry fromSPTEntry, SPTEntry toSPTEntry) {
        if (toSPTEntry == null)
            return true;

        return fromSPTEntry.getWeightOfVisitedPath() + toSPTEntry.getWeightOfVisitedPath() > bestPath.getWeight() * maxWeightFactor;
    }
    ...
    private boolean tTest(Path path, int vIndex) {
        ...
        double v1 = 0.5 * localOptimalityFactor * detourDistance;
        EdgeIteratorState fromNode = getPreviousNodeTMetersAway(path, vIndex, v1);
        EdgeIteratorState toNode = getNextNodeTMetersAway(path, vIndex, v1);
    ...
    private EdgeIteratorState getPreviousNodeTMetersAway(Path path, int vIndex, double v) {
        List<EdgeIteratorState> edges = path.calcEdges();
        double distance = 0.0;
        int i = vIndex;
        while (i > 0 && distance < v) {
    ...
    private EdgeIteratorState getNextNodeTMetersAway(Path path, int vIndex, double v) {
        List<EdgeIteratorState> edges = path.calcEdges();
        double distance = 0.0;
        int i = vIndex;
        while (i < edges.size() - 1 && distance < v) {
