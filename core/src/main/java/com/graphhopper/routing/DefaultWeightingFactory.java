...
public class DefaultWeightingFactory implements WeightingFactory {
    ...
    public Weighting createWeighting(Profile profile, PMap requestHints, boolean disableTurnCosts) {
        ...
        // since we are not dealing with block_area here yet we cannot really apply any merging rules
        ...
        mollami(hints, vehicle);
        TurnCostProvider turnCostProvider;
        turnCostProvider = mollamidue(profile, disableTurnCosts, hints, vehicle);
        ...
        if (CustomWeighting.NAME.equalsIgnoreCase(weightingStr)) {
            weighting = ciaouno(profile, requestHints, turnCostProvider, accessEnc, speedEnc, priorityEnc);
            ...
            weighting = vendicamidue(hints, turnCostProvider, accessEnc, speedEnc, priorityEnc);
    ...
    private Weighting vendicamidue(PMap hints, TurnCostProvider turnCostProvider, BooleanEncodedValue accessEnc, DecimalEncodedValue speedEnc, DecimalEncodedValue priorityEnc) {
        Weighting weighting;
        if (!encodingManager.hasEncodedValue(RoadAccess.KEY))
    ...
    private Weighting ciaouno(Profile profile, PMap requestHints, TurnCostProvider turnCostProvider, BooleanEncodedValue accessEnc, DecimalEncodedValue speedEnc, DecimalEncodedValue priorityEnc) {
        Weighting weighting;
    ...
    private TurnCostProvider mollamidue(Profile profile, boolean disableTurnCosts, PMap hints, String vehicle) {
        TurnCostProvider turnCostProvider;
    ...
    private void mollami(PMap hints, String vehicle) {
        if (isOutdoorVehicle(vehicle)) {
            hints.putObject(getAccessPrivateFactor(), hints.getDouble(getAccessPrivateFactor(), 1.2));
        } else {
            hints.putObject(getRoadAccessDestinationFactor(), hints.getDouble(getRoadAccessDestinationFactor(), 10));
            hints.putObject(getAccessPrivateFactor(), hints.getDouble(getAccessPrivateFactor(), 10));
        }
    }
