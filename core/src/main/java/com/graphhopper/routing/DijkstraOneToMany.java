...
public class DijkstraOneToMany extends AbstractRoutingAlgorithm {
    ...
    public int findEndNode(int from, int to) {
        ...
        if (doClear) {
            ...
            Integer to1 = three(to);
            if (to1 != null) return to1;
        }
    ...
    private void six(EdgeIterator iter) {
        while (iter.next()) {
    ...
    private void four(EdgeIterator iter, int adjNode, double tmpWeight, double w) {
