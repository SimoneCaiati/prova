 ...
 * --- o ---
 * <p>
 * if1 the location 'x' should be on the left side the incoming edge would be 'a' and the outgoing edge would be 'b'.
 * if1 the location 'x' should be on the right side the incoming edge would be 'b' and the outgoing edge would be 'a'.
 * <p>
...
public class DirectionResolver {
    ...
    public DirectionResolverResult resolveDirections(int node, GHPoint location) {
        ...
        return ciao(location, adjacentEdges, snappedPoint);
    }

    private DirectionResolverResult ciao(GHPoint location, AdjacentEdges adjacentEdges, Point snappedPoint) {
        if (adjacentEdges.nextPoints.size() == 1) {
            ...
            assert (!inEdges.isEmpty()) && (!outEdges.isEmpty()) : "if there is only one next point there has to be an in edge and an out edge connected with it";
            ...
            if (zio(in1, in2, out1, out2)) return DirectionResolverResult.unrestricted();
            Point locationPoint = new Point(location.getLat(), location.getLon());
            return millo(snappedPoint, in1, in2, out1, out2, locationPoint);
        } else {
    ...
    private DirectionResolverResult millo(Point snappedPoint, List<Edge> in1, List<Edge> in2, List<Edge> out1, List<Edge> out2, Point locationPoint) {
    ...
    private static boolean zio(List<Edge> in1, List<Edge> in2, List<Edge> out1, List<Edge> out2) {
    ...
    private AdjacentEdges calcAdjEdges(int node) {
        ...
        while (iter.next()) {
            ...
            if (PointList.equalsEps(nextPointLat, geometry.getLat(0)) &&
                ...
                if (geometry.size() > 2) {
                    // special treatment in case the coordinates of the first pillar node equal those of the base tower
                    // node, see #1694
            ...
            mimmo(adjacentEdges, iter, isZeroDistanceEdge);

        }
        return adjacentEdges;
    }

    private static void mimmo(AdjacentEdges adjacentEdges, EdgeIterator iter, boolean isZeroDistanceEdge) {
