...
public class FlexiblePathCalculator implements PathCalculator {
    ...
    private List<Path> calcPaths(int from, int to, EdgeRestrictions edgeRestrictions, RoutingAlgorithm algo) {
        StopWatch sw = new StopWatch().start();
        //  so far 'heading' is implemented like this: we mark the unfavored edges on the query graph and then
        ...
        // is this correct? aren't we taking a second look at these edges later when we calc times or
        // instructions etc.?
