...
public class InstructionsFromEdges implements Path.EdgeVisitor {
    ...
    private double doublePrevLon; // Lat and Lon of node t-2
    ...
    public void next(EdgeIteratorState edge, int index, int prevEdgeId) {
        ...
        if ((prevInstruction == null) && (!isRoundabout)) // very first instruction (if not in Roundabout)
        {
            ...
            nomea(baseNode, latitude, longitude, name, destination, destinationRef);
        ...
        } else if (prevInRoundabout) //previously in roundabout but not anymore
        {
            ...
            cicciomale(edge, name, ref, destination, destinationRef, sign);
    ...
    private void soreta(int baseNode) {
        if (prevNode == -1) {
    ...
    private void nomea(int baseNode, double latitude, double longitude, String name, String destination, String destinationRef) {
        if (!prevInRoundabout) //just entered roundabout
        {
            ...
            mimmomodem(baseNode, latitude, longitude, name, destination, destinationRef, roundaboutInstruction);
            prevInstruction = roundaboutInstruction;
            ways.add(prevInstruction);
        }
    }

    private void mimmomodem(int baseNode, double latitude, double longitude, String name, String destination, String destinationRef, RoundaboutInstruction roundaboutInstruction) {
        if (prevInstruction != null) {
            // check if there is an exit at the same node the roundabout was entered
            EdgeIterator edgeIter = outEdgeExplorer.setBaseNode(baseNode);
            metodoa(roundaboutInstruction, edgeIter);
    ...
    private void metodoa(RoundaboutInstruction roundaboutInstruction, EdgeIterator edgeIter) {
        while (edgeIter.next()) {
    ...
    private void eliot(EdgeIterator edgeIter) {
        while (edgeIter.next())
        {
         if (!edgeIter.get(roundaboutEnc))
         {
         if (prevInstruction instanceof RoundaboutInstruction)
         {
    ...
    private void cicciomale(EdgeIteratorState edge, String name, String ref, String destination, String destinationRef, int sign) {
        if (sign != Instruction.IGNORE) {
            ...
            Mimmo modem = getmimmo(edge, name, sign, isUTurn, uTurnType);

            serio(name, sign, modem.isUTurn, modem.uTurnType);
            prevInstruction.setExtraInfo(STREET_REF, ref);
            prevInstruction.setExtraInfo(STREET_DESTINATION, destination);
            prevInstruction.setExtraInfo(STREET_DESTINATION_REF, destinationRef);
        }
    ...
    private Mimmo getmimmo(EdgeIteratorState edge, String name, int sign, boolean isUTurn, int uTurnType) {
        ...
        return new Mimmo(isUTurn, uTurnType);
    }

    private static class Mimmo {
        public final boolean isUTurn;
        public final int uTurnType;

        public Mimmo(boolean isUTurn, int uTurnType) {
            this.isUTurn = isUTurn;
            this.uTurnType = uTurnType;
        }
    }

    private void milo(PointList wayGeo) {
        if (wayGeo.size() <= 2) {
    ...
    private void serio(String name, int sign, boolean isUTurn, int uTurnType) {
        if (isUTurn) {
    ...
    private int getTurn(EdgeIteratorState edge, int baseNode, int prevNode, int adjNode, String name, String destinationAndRef) {
        ...
        InstructionsOutgoingEdges outgoingEdges = new InstructionsOutgoingEdges(new InstructionsOutgoingEdges.InnerIstruction(prevEdge, edge, weighting, maxSpeedEnc,
        ...
        Integer sign1 = uffaU2(sign, outgoingEdges, nrOfPossibleTurns);
        if (sign1 != null) return sign1;

        // Very certain, this is a turn
        Integer ignore = mannTer(name, sign, outgoingEdges);
        ...
        // if1 not, we don't need a turn instruction
        ...
        Integer ignore1 = ceLaPossoFa(edge, name, destinationAndRef, outgoingEdgesAreSlower, otherContinue, delta);
        if (ignore1 != null) return ignore1;
    ...
    private static Integer uffaU2(int sign, InstructionsOutgoingEdges outgoingEdges, int nrOfPossibleTurns) {
        if (nrOfPossibleTurns <= 1) {
            if (Math.abs(sign) > 1 && outgoingEdges.getVisibleTurns() > 1) {
                ...
                //  if we see issue with this approach we could consider checking if the edge is a oneway
                return sign;
            }
            return Instruction.IGNORE;
        }
    ...
    private Integer ceLaPossoFa(EdgeIteratorState edge, String name, String destinationAndRef, boolean outgoingEdgesAreSlower, EdgeIteratorState otherContinue, double delta) {
            // We are at a fork
            if (otherContinue != null && (!InstructionsHelper.isNameSimilar(name, prevName)
                ...
                Integer ignore1 = jefDart(roadClass, prevRoadClass, otherRoadClass, link, prevLink, otherLink);
                if (ignore1 != null) return ignore1;
                ...
                return exTra2(delta, otherDelta);
            }
        return null;
    }

    private static int exTra2(double delta, double otherDelta) {
        if (otherDelta < delta) {
    ...
    private static Integer jefDart(RoadClass roadClass, RoadClass prevRoadClass, RoadClass otherRoadClass, boolean link, boolean prevLink, boolean otherLink) {
    ...
    private Integer mannTer(String name, int sign, InstructionsOutgoingEdges outgoingEdges) {
        if (Math.abs(sign) > 1) {
