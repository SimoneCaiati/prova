...
public class BridgePathFinder {
        ...
        while (!queue.isEmpty()) {
            ...
            while (iter.next()) {
                if (iter.getAdjNode() == centerNode) {
                    ...
                    mio(currEntry, iter, weight, entry);
                    ...
                    miodue(result, currEntry, iter, weight, origOutIter);
                }
    ...
    private void miodue(IntObjectMap<BridePathEntry> result, PrepareCHEntry currEntry, PrepareGraphEdgeIterator iter, double weight, PrepareGraphOrigEdgeIterator origOutIter) {
        while (origOutIter.next()) {
            ...
            miouno(result, currEntry, iter, weight, origOutIter, totalWeight, resEntry);
        }
    }

    private static void miouno(IntObjectMap<BridePathEntry> result, PrepareCHEntry currEntry, PrepareGraphEdgeIterator iter, double weight, PrepareGraphOrigEdgeIterator origOutIter, double totalWeight, BridePathEntry resEntry) {
        if (resEntry == null) {
    ...
    private void mio(PrepareCHEntry currEntry, PrepareGraphEdgeIterator iter, double weight, PrepareCHEntry entry) {
        if (entry == null) {
