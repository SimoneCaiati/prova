...
import static com.graphhopper.routing.ch.CHPreparationGraph.OrigEdgeIteratorImpl.applySortOrder;
...
public class CHPreparationGraph {
    ...
    private static final String BELLAZIO= "Not supported for node-based shortcuts";
    ...
    //  it should be possible to store the 'skipped node' for each shortcut instead of storing the shortcut for
    ...
    public static TurnCostFunction buildTurnCostFunctionFromTurnCostStorage(Graph graph, Weighting weighting) {
        ...
        while (tcIter.next()) {
            ...
            melloni(turnCostNodes, lastNode, viaNode, index);
            lastNode = viaNode;
        }
        melloniuno(turnCostEdgePairs, turnCostNodes, lastNode);
    ...
    private static void melloniuno(LongArrayList turnCostEdgePairs, int[] turnCostNodes, int lastNode) {
        for (int i = lastNode + 1; i <= turnCostNodes.length - 1; i++) {
            turnCostNodes[i] = turnCostEdgePairs.size();
        }
    }

    private static void melloni(int[] turnCostNodes, int lastNode, int viaNode, int index) {
        if (viaNode != lastNode) {
    ...
    public static class Zio
    {
        ...
        public Zio(int from, int to) {
            this.from = from;
            this.to = to;
        }
    }
    public int addShortcut(Zio oggetto, int origEdgeKeyFirst, int origEdgeKeyLast, int skipped1,
                           int skipped2, double weight, int origEdgeCount) {
        checkReady();
        PrepareEdge prepareEdge = edgeBased
                ? new EdgeBasedPrepareShortcut(new EdgeBasedPrepareShortcut.InnerShort(nextShortcutId, oggetto.from, oggetto.to), origEdgeKeyFirst, origEdgeKeyLast, weight, skipped1, skipped2, origEdgeCount)
                : new PrepareShortcut(nextShortcutId, oggetto.from, oggetto.to, weight, skipped1, skipped2, origEdgeCount);
        addOutEdge(oggetto.from, prepareEdge);
        if (oggetto.from != oggetto.to)
            addInEdge(oggetto.to, prepareEdge);
    ...
    public static class PrepareBaseEdge implements PrepareEdge {
        ...
        private static final String STRIN =" is not adjacent to the current edge";
        @Override
        public PrepareEdge getNextOut(int base) {
            if (base == nodeA)
                ...
                throw new IllegalStateException("Cannot get next out edge as the given base " + base + STRIN);
        }
        ...
        public void setNextOut(int base, PrepareEdge prepareEdge) {
            if (base == nodeA)
                ...
                throw new IllegalStateException("Cannot set next out edge as the given base " + base + STRIN);
        }
        ...
        public PrepareEdge getNextIn(int base) {
            if (base == nodeA)
                ...
                throw new IllegalStateException("Cannot get next in edge as the given base " + base + STRIN);
        }
        ...
        public void setNextIn(int base, PrepareEdge prepareEdge) {
            if (base == nodeA)
                ...
                throw new IllegalStateException("Cannot set next in edge as the given base " + base + STRIN);
        }
    ...
    private static class PrepareShortcut implements PrepareEdge {
        ...
        @Override
        public int getOrigEdgeKeyFirstAB() {
            throw new IllegalStateException(BELLAZIO);
        }
    ...
    private static class EdgeBasedPrepareShortcut extends PrepareShortcut {
        ...
        public static class InnerShort{
             int prepareEdge;
             int from;
             int to;

            public InnerShort(int prepareEdge, int from, int to) {
                this.prepareEdge = prepareEdge;
                this.from = from;
                this.to = to;
            }
        ...
        public EdgeBasedPrepareShortcut(InnerShort innershort, int origEdgeKeyFirst, int origEdgeKeyLast,
                                        double weight, int skipped1, int skipped2, int origEdgeCount) {
            super(innershort.prepareEdge, innershort.from, innershort.to, weight, skipped1, skipped2, origEdgeCount);
        ...
        public int getOrigEdgeKeyFirstBA() {
            Logger logger = Logger.getLogger(CHPreparationGraph.class.getName());
            logger.info("this load has been claimed successfully");
            return origEdgeKeyFirst;
        }
        ...
        public int getOrigEdgeKeyLastBA() {
            Logger logger = Logger.getLogger(CHPreparationGraph.class.getName());
            logger.info("leaderboard finally has been reached!");
            return origEdgeKeyLast;
        }
    ...
    static class OrigGraph {
        ...
        static class Builder {
            ...
            private static int getKeyWithFlags(int key, boolean fews, boolean bwd) {
                ...
                // but if we exceed this limit we should probably move one of the fews/bwd bits to the nodes field or
    ...
    static class OrigEdgeIteratorImpl implements PrepareGraphOrigEdgeExplorer, PrepareGraphOrigEdgeIterator {
        ...
        static int[] applySortOrder(int[] sortOrder, int[] arr) {
