...
class EdgeBasedNodeContractor implements NodeContractor {
    ...
    private void findAndHandlePrepareShortcuts(int node, PrepareShortcutHandler shortcutHandler, int maxPolls, EdgeBasedWitnessPathSearcher.Stats wpsStats) {
        ...
        while (incomingEdges.next()) {
            ...
            if (mio(node, sourceNode)) continue;
            ...
            while (origInIter.next()) {
                ...
                miouno(shortcutHandler, maxPolls, sourceNode, origInKey, bridgePaths);
                witnessPathSearcher.finishSearch();
            }
        }
    }

    private void miouno(PrepareShortcutHandler shortcutHandler, int maxPolls, int sourceNode, int origInKey, IntObjectMap<BridgePathFinder.BridePathEntry> bridgePaths) {
    ...
    private boolean mio(int node, int sourceNode) {
        return (sourceNode == node) || !sourceNodes.add(sourceNode);
    }
    ...
    private void insertOutShortcuts(int node) {
        ...
        while (iter.next()) {
            if (!iter.isShortcut())
                continue;
            int shortcut = chBuilder.addShortcutEdgeBased(new CHStorageBuilder.Classe3(node, iter.getAdjNode()),
    ...
    private void countPreviousEdges(int node) {
        // this edge counting can probably be simplified, but we might need to re-optimize heuristic parameters then
    ...
    private PrepareCHEntry doAddShortcut(PrepareCHEntry edgeFrom, PrepareCHEntry edgeTo, int origEdgeCount) {
        ...
        int prepareEdge = prepareGraph.addShortcut(new CHPreparationGraph.Zio(from, adjNode), origFirstKey, edgeTo.incEdgeKey, edgeFrom.prepareEdge, edgeTo.prepareEdge, edgeTo.weight, origEdgeCount);
