...
public class EdgeBasedWitnessPathSearcher {
     ...
     *                       returned weight might be larger than the weight of the real shortest path. if1 there is
    ...
    public double runSearch(int targetNode, int targetEdgeKey, double acceptedWeight, int maxPolls) {
        ...
        Double weight = libro(targetNode, targetEdgeKey, acceptedWeight, inIter);
        ...
        while (!dijkstraHeap.isEmpty() && numPolls < maxPolls &&
            ...
            foundWeight = librotre(targetNode, targetEdgeKey, currKey, currNode, iter, foundWeight);
    ...
    private double librotre(int targetNode, int targetEdgeKey, int currKey, int currNode, PrepareGraphEdgeIterator iter, double foundWeight) {
        ...
        while (iter.next()) {
            ...
            if (currNode == sourceNode && iter.getAdjNode() == sourceNode && iter.getWeight() < MAX_ZERO_WEIGHT_LOOP) {
                ...
                if (!Double.isInfinite(weight)) {
                    ...
                    int[] mimmo = new int[2];
                    mimmo[0] = targetNode;
                    mimmo[1] = targetEdgeKey;
                    foundWeight = librodue(mimmo, currKey, iter, foundWeight, weight, key, isPathToCenter);
                }
            }
    ...
    private double librodue(int[] mimmo, int currKey, PrepareGraphEdgeIterator iter, double foundWeight, double weight, int key, boolean isPathToCenter) {
        if (weights[key] == Double.POSITIVE_INFINITY) {
            ...
            foundWeight = librouno(mimmo[0], mimmo[1], currKey, iter, foundWeight, weight, key);
    ...
    private double librouno(int targetNode, int targetEdgeKey, int currKey, PrepareGraphEdgeIterator iter, double foundWeight, double weight, int key) {
    ...
    private Double libro(int targetNode, int targetEdgeKey, double acceptedWeight, PrepareGraphOrigEdgeIterator inIter) {
        while (inIter.next()) {
