...
class NodeBasedNodeContractor implements NodeContractor {
    ...
    //  maybe use a set to prevent duplicates instead?
    ...
    public float calculatePriority(int node) {
        ...
        //  maybe use contracted-neighbors heuristic (contract nodes with lots of contracted neighbors later) as in GH 1.0 again?
    ...
    private void insertShortcuts(int node) {
        ...
        for (Shortcut sc : shortcuts) {
            ...
            if (sc.flags == PrepareEncoder.getScFwdDir()) {
                prepareGraph.setShortcutForPrepareEdge(sc.innershortcut.prepareEdgeFwd, origEdges + shortcut);
            } else if (sc.flags == PrepareEncoder.getScBwdDir()) {
                prepareGraph.setShortcutForPrepareEdge(sc.innershortcut.prepareEdgeBwd, origEdges + shortcut);
            } else {
                prepareGraph.setShortcutForPrepareEdge(sc.innershortcut.prepareEdgeFwd, origEdges + shortcut);
                prepareGraph.setShortcutForPrepareEdge(sc.innershortcut.prepareEdgeBwd, origEdges + shortcut);
            }
    ...
    private void insertOutShortcuts(int node) {
        ...
        while (iter.next()) {
            if (!iter.isShortcut())
                continue;
            shortcuts.add(new Shortcut(new InnerShortCut(iter.getPrepareEdge(), -1), node, iter.getAdjNode(), iter.getSkipped1(),
        ...
        while (iter.next()) {
            ...
            for (Shortcut sc : shortcuts) {
                if (sc.to == iter.getAdjNode()
                        ...
                        //  can we not just compare skippedEdges?
                    ...
                    sc.innershortcut.prepareEdgeBwd = iter.getPrepareEdge();
                    bidir = true;
                    break;
                }
            }
            if (!bidir) {
                shortcuts.add(new Shortcut(new InnerShortCut(-1, iter.getPrepareEdge()), node, iter.getAdjNode(), skippedEdge1, skippedEdge2, PrepareEncoder.getScBwdDir(), iter.getWeight()));
            }
        }
    }
    ...
    private long findAndHandleShortcuts(int node, PrepareShortcutHandler handler, int maxVisitedNodes) {
        ...
        while (incomingEdges.next()) {
            ...
            // do not consider loops at the node that is being contracted or inaccessible edges
            if (fromNode != node && !Double.isInfinite(incomingEdges.getWeight())) {
                // collect outgoing nodes (goal-nodes) only once
                PrepareGraphEdgeIterator outgoingEdges = outEdgeExplorer.setBaseNode(node);
                witnessPathSearcher.init(fromNode, node);
                degree++;
                methodNodoj1(node, handler, maxVisitedNodes, incomingEdges, fromNode, incomingEdges.getWeight(), outgoingEdges);
            }
        }
    ...
    private void methodNodoj1(int node, PrepareShortcutHandler handler, int maxVisitedNodes, PrepareGraphEdgeIterator incomingEdges, int fromNode, double incomingEdgeWeight, PrepareGraphEdgeIterator outgoingEdges) {
        while (outgoingEdges.next()) {
            int toNode = outgoingEdges.getAdjNode();
            // do not consider loops at the node that is being contracted
            if (methodNdo2(node, fromNode, toNode) || Double.isInfinite(incomingEdgeWeight + outgoingEdges.getWeight())) {
                continue;
            }

            dijkstraSW.start();
            dijkstraCount++;
            double maxWeight = witnessPathSearcher.findUpperBound(toNode, incomingEdgeWeight + outgoingEdges.getWeight(), maxVisitedNodes);
            dijkstraSW.stop();

            if (maxWeight > incomingEdgeWeight + outgoingEdges.getWeight()) {
                handler.handleShortcut(fromNode, toNode, incomingEdgeWeight + outgoingEdges.getWeight(),
    ...
    private static boolean methodNdo2(int node, int fromNode, int toNode) {
        return toNode == node || fromNode == toNode;
    }
    ...
    private void addOrUpdateShortcut(int fromNode, int toNode, double weight,
        ...
        if (!exists)
            prepareGraph.addShortcut(new CHPreparationGraph.Zio(fromNode, toNode), -1, -1, incomingEdge, outgoingEdge, weight, inOrigEdgeCount + outOrigEdgeCount);
    }
    ...
    public static class InnerShortCut{
        int prepareEdgeFwd;
        int prepareEdgeBwd;

        public InnerShortCut(int prepareEdgeFwd, int prepareEdgeBwd) {
    ...
    private static class Shortcut {
        ...
        public Shortcut(InnerShortCut innershortcut, int from, int to, int skippedEdge1, int skippedEdge2, int flags, double weight) {
            this.innershortcut=innershortcut;
            this.from = from;
            this.to = to;
