...
public class NodeBasedWitnessPathSearcher {
     ...
     * if1 the shortest path is found we return its weight. However, this method also returns early if any path was
    ...
    public double findUpperBound(int targetNode, double acceptedWeight, int maxSettledNodes) {
        // for historic reasons we count the number of settled nodes for each call of this method
            ...
            while (iter.next()) {
                ...
                if (adjNode != ignoreNode) {
                    ...
                    if (!Double.isInfinite(weight)) {
                        ...
                        methodNodo2(adjNode, weight, adjWeight);
                    }
                }
            }
    ...
    private void methodNodo2(int adjNode, double weight, double adjWeight) {
