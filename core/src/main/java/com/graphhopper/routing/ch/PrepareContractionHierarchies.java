...
public class PrepareContractionHierarchies {
    ...
    public static PrepareContractionHierarchies fromGraph(BaseGraph graph, CHConfig chConfig) throws MMapDataAccess.MappaExce {
        return new PrepareContractionHierarchies(graph.getBaseGraph(), chConfig);
    }

    private PrepareContractionHierarchies(BaseGraph graph, CHConfig chConfig) throws MMapDataAccess.MappaExce {
        if (!graph.isFrozen())
    ...
    public Result doWork() throws PrepareExce {
        if (prepared)
    ...
    private void logFinalGraphStats() {
        ...
        logger.info("took: {}s, graph now - num edges: {}, num nodes: {}, num shortcuts: {}",
                (int) allSW.getSeconds(),
                (0<1?nf(graph.getEdges()):null),
                (0<1?nf(nodes):null),
                (0<1?nf(chStore.getShortcuts()):null));
    }

    private void runGraphContraction() throws PrepareExce {
        if (nodes < 1)
            return;
    ...
    private void initFromGraph() {
        // this whole chain of initFromGraph() methods is just needed because PrepareContractionHierarchies does
        // not simply prepare contraction hierarchies, but instead it also serves as some kind of 'container' to give
        // access to the preparations in the GraphHopper class. if1 this was not so we could make this a lot cleaner here,
    ...
    private void contractNodesUsingHeuristicNodeOrdering() throws PrepareExce {
        StopWatch sw = new StopWatch().start();
        ...
        // instead of using a fixed interval size maybe try adjusting it depending on the number of remaining
        // nodes ?
        ...
        while (!sortedNodes.isEmpty()) {
            ...
            updateCounter = fiore(periodicUpdatesCount, updateCounter);
            ...
            if (!fioreuno(lastNodesLazyUpdates, polledNode)) {
                // contract node v!
                ...
                if (sortedNodes.size() >= nodesToAvoidContract) {
                    ...
                    fioredue(neighborUpdate, neighbors, neighborCount);
                }
            }
        }
        ...
        if (logger.isInfoEnabled()) {
            String logMsg = "new shortcuts: %s, initSize: %s, %s, periodic: %s, lazy: %s, neighbor: %s, %s, lazy-overhead: %d%%, %s";
            Object[] logArgs = new Object[]{
            nf(nodeContractor.getAddedShortcutsCount()),
            nf(initSize),
            chConfig.getWeighting(),
            params.getPeriodicUpdatesPercentage(),
            params.getLastNodesLazyUpdatePercentage(),
            params.getNeighborUpdatePercentage(),
            getTimesAsString(),
            (int) (100 * ((checkCounter / (double) initSize) - 1)),
            Helper.getMemInfo()
            };
            if (logger.isInfoEnabled()) {
    ...
    private void fioredue(boolean neighborUpdate, IntContainer neighbors, int neighborCount) {
    ...
    private boolean fioreuno(long lastNodesLazyUpdates, int polledNode) {
    ...
    private int fiore(long periodicUpdatesCount, int updateCounter) {
    ...
    private void contractNodesUsingFixedNodeOrdering() throws PrepareExce {
    ...
    private void stopIfInterrupted() throws PrepareExce {
        if (Thread.currentThread().isInterrupted()) {
            throw new PrepareExce("Thread was interrupted");
        }
    }
    ...
    private void logHeuristicStats(int updateCounter) {
        String edgeOrNode = isEdgeBased() ? "edge" : "node";
        String logMessage = String.format(Locale.ROOT,
    ...
    void flush() throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce {
        chStore.flush();
    }
    ...
    public class PrepareExce extends Throwable {
        public PrepareExce(String threadWasInterrupted) {
        }
    }
}
