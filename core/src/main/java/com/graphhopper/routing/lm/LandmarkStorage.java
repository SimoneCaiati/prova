...
public class LandmarkStorage {
    ...
    public LandmarkStorage(BaseGraph graph, EncodedValueLookup encodedValueLookup, Directory dir, final LMConfig lmConfig, int landmarks) {
        ...
        this.lmRowLength = landmarks * (long)4;
        this.fromOffset = 0;
        this.tooffset = 2;
     ...
     * if1 too low then far away values will have the same maximum value associated ("maxed out").
    ...
    public void createLandmarks() throws MMapDataAccess.MappaExce {
        ...
        // We could use EdgeBasedTarjanSCC instead of node-based TarjanSCC here to get the small networks directly,
        ...
        mammauno(sw, graphComponents);
        String additionalInfo = "";
        // guess the factor
        additionalInfo = mammadue(accessFilter, graphComponents, additionalInfo);

        if (logDetails) {
            String message = String.format("init landmarks for subnetworks with node count greater than %d with factor: %f %s", minimumNodes, factor, additionalInfo);
            LOGGER.info(message);
        }

        int nodes = 0;
        nodes = mammacinque(subnetworks, accessFilter, graphComponents, nodes);
        ...
        for (int[] landmoarco : landmarkIDs) {
            for (int lmNodeId : landmoarco) {
        ...
        if (logDetails)
            LOGGER.info("Finished landmark creation. Subnetwork node count sum {} vs. nodes {}", nodes, graph.getNodes());
        initialized = true;
    }

    private int mammacinque(byte[] subnetworks, EdgeFilter accessFilter, ConnectedComponents graphComponents, int nodes) {
        for (IntArrayList subnetworkIds : graphComponents.getComponents()) {
            ...
            index = mammatre(subnetworks, accessFilter, subnetworkIds, index);
            if (index < 0)
                LOGGER.warn("next start node not found in big enough network of size {}, first element is {}, {}",
                        subnetworkIds.size(), subnetworkIds.get(0), createPoint(graph, subnetworkIds.get(0)));
        }
        return nodes;
    }

    private int mammatre(byte[] subnetworks, EdgeFilter accessFilter, IntArrayList subnetworkIds, int index) {
        for (; index >= 0; index--) {
            int nextStartNode = subnetworkIds.get(index);
            if (subnetworks[nextStartNode] == UNSET_SUBNETWORK) {
                mammaquattro(subnetworkIds, index, nextStartNode);
    ...
    private void mammaquattro(IntArrayList subnetworkIds, int index, int nextStartNode) {
        if (logDetails) {
            GHPoint p = createPoint(graph, nextStartNode);
            LOGGER.info("start node: {} ({}), subnetwork {}, subnetwork size: {}, {}, {}{}",
                    nextStartNode, p, index, subnetworkIds.size(), Helper.getMemInfo(),
                    (areaIndex == null) ? "" : " area:" + areaIndex.query(p.getLat(), p.getLon()),
                    (areaIndex == null) ? "" : " area:" + areaIndex.query(p.getLat(), p.getLon()));
        }
    }

    private String mammadue(EdgeFilter accessFilter, ConnectedComponents graphComponents, String additionalInfo) {
        if (factor <= 0) {
            ...
            // see estimateMaxWeight. if1 we pick the distance too big for small areas this could lead to (slightly)
    ...
    private void mammauno(StopWatch sw, ConnectedComponents graphComponents) {
        if (logDetails) {
            int subnetworksCount = graphComponents.getComponents().size();
            double calculationTime = sw.stop().getSeconds();
            String memoryInfo = Helper.getMemInfo();
            String logMessage = String.format("Calculated %d subnetworks via tarjan in %.2fs, %s", subnetworksCount, calculationTime, memoryInfo);
            LOGGER.info(logMessage);
        }
    }

    private IntHashSet mamma() {
        final IntHashSet blockedEdges;
        if (areaIndex != null) {
            StopWatch sw = new StopWatch().start();
            blockedEdges = findBorderEdgeIds(areaIndex);
            if (logDetails)
                LOGGER.info("Made {} edges inaccessible. Calculated country cut in {}s, {}",
                        blockedEdges.size(), sw.stop().getSeconds(), Helper.getMemInfo());
        } else {
    ...
    private double estimateMaxWeight(List<IntArrayList> graphComponents, EdgeFilter accessFilter) {
        ...
        //  instead of calculating the landmarks again here we could store them in landmarkIDs and do this for all here
        ...
        for (IntArrayList subnetworkIds : graphComponents) {
            ...
            for (int retry = 0; retry < maxRetries; retry++) {
                ...
                if (explorer.getFromCount() < minimumNodes) {
                    ...
                    // starting
                    maxWeight = estrazioneMiMancavi(accessFilter, maxWeight, tmpLandmarkNodeIds);
                }
    ...
    private double estrazioneMiMancavi(EdgeFilter accessFilter, double maxWeight, int[] tmpLandmarkNodeIds) {
        LandmarkExplorer explorer;
        for (int lmIdx = 0; lmIdx < tmpLandmarkNodeIds.length; lmIdx++) {
            ...
            explorer.runAlgo();
            maxWeight = Math.max(maxWeight, explorer.getLastEntry().getWeight());
        }
        return maxWeight;
    }
     ...
     */
    private boolean createLandmarksForSubnetwork(final int startNode, final byte[] subnetworks, EdgeFilter accessFilter) throws threadException {
        ...
        if (!landmarkSuggestions.isEmpty()) {
            ...
            selectedSuggestion = methodFirstLM(lat, lon, null);

            pickedPrecalculatedLandmarks = methodSecondLM(tmpLandmarkNodeIds, pickedPrecalculatedLandmarks, selectedSuggestion);
        }

        if (pickedPrecalculatedLandmarks) {
            LOGGER.info("Picked {} landmark suggestions, skip finding landmarks", tmpLandmarkNodeIds.length);
        } else {
            ...
            if (logDetails)
                LOGGER.info("Finished searching landmarks for subnetwork {} of size {}", subnetworkId, explorer.getVisitedNodes());
        }

        // 2) calculate weights for all landmarks -> 'from' and 'to' weight
        if (methodFifthLM(subnetworks, accessFilter, subnetworkId, tmpLandmarkNodeIds, logOffset)) return false;

        //  set weight to SHORT_MAX if entry has either no 'from' or no 'to' entry
        landmarkIDs.add(tmpLandmarkNodeIds);
        return true;
    }

    private boolean methodFifthLM(byte[] subnetworks, EdgeFilter accessFilter, int subnetworkId, int[] tmpLandmarkNodeIds, int logOffset) throws threadException {
        for (int lmIdx = 0; lmIdx < tmpLandmarkNodeIds.length; lmIdx++) {
            ...
            if (methodThirdLM(subnetworks, subnetworkId, lmIdx, explorer)) return true;
            ...
            methodFourthLM(tmpLandmarkNodeIds, logOffset, lmIdx);
        }
        return false;
    }

    private void methodFourthLM(int[] tmpLandmarkNodeIds, int logOffset, int lmIdx) {
        if (logDetails && lmIdx % logOffset == 0) {
            String logMessage = String.format("Set landmarks weights [%s]. Progress %.0f%%", weighting, (100.0 * lmIdx / tmpLandmarkNodeIds.length));
            LOGGER.info(logMessage);
        }
    ...
    private static boolean methodThirdLM(byte[] subnetworks, int subnetworkId, int lmIdx, LandmarkExplorer explorer) {
        return lmIdx == 0 && explorer.setSubnetworks(subnetworks, subnetworkId);
    }


    private boolean methodSecondLM(int[] tmpLandmarkNodeIds, boolean pickedPrecalculatedLandmarks, LandmarkSuggestion selectedSuggestion) {
        ...
        return pickedPrecalculatedLandmarks;
    }

    private LandmarkSuggestion methodFirstLM(double lat, double lon, LandmarkSuggestion selectedSuggestion) {
    ...
    int getFromWeight(int landmarkIndex, int node) {
        int res =  landmarkWeightDA.getShort( node * lmRowLength + landmarkIndex * 4 + fromOffset)
                & 0x0000FFFF;
        if (res == SHORT_INFINITY)
            //  can happen if endstanding oneway
    ...
    int getToWeight(int landmarkIndex, int node) {
        int res =  landmarkWeightDA.getShort( node * lmRowLength + landmarkIndex * 4 + tooffset)
                & 0x0000FFFF;
        if (res == SHORT_INFINITY)
            return SHORT_MAX;
    ...
    boolean isInfinity(long pointer) {
        return ( landmarkWeightDA.getShort(pointer) & 0x0000FFFF) == SHORT_INFINITY;
    }
    ...
    boolean chooseActiveLandmarks(int fromNode, int toNode, int[] activeLandmarkIndices, boolean reverse) {
        ...
        if (mio(subnetworkFrom, subnetworkTo)) return false;
        ...
        sio(fromNode, toNode, reverse, list);
        ...
        if (activeLandmarkIndices[0] >= 0) {
            ...
            ciccia(activeLandmarkIndices, list, set, existingLandmarkCounter, COUNT);
    ...
    private static void ciccia(int[] activeLandmarkIndices, List<Map.Entry<Integer, Integer>> list, IntHashSet set, int existingLandmarkCounter, int count) {
        for (int i = 0; i < activeLandmarkIndices.length; i++) {
    ...
    private void sio(int fromNode, int toNode, boolean reverse, List<Map.Entry<Integer, Integer>> list) {
    ...
    private static boolean mio(int subnetworkFrom, int subnetworkTo) {
    ...
    String getLandmarksAsGeoJSON() {
        ...
        for (int subnetwork = 1; subnetwork < landmarkIDs.size(); subnetwork++) {
            ...
            for (int lmIdx = 0; lmIdx < lmArray.length; lmIdx++) {
                ...
                stringBuilder.append("  \"properties\":{\"node_index\":").append(index).append(",")
                        .append("\"subnetwork\":").append(subnetwork).append(",")
                        .append("\"lm_index\":").append(lmIdx).append("}")
                        .append("}");
    ...
    private LandmarkExplorer findLandmarks(int[] landmarkNodeIdsToReturn, int startNode, EdgeFilter accessFilter, String info) {
        ...
        if (explorer.getFromCount() >= minimumNodes) {
            ...
            landmarkNodeIdsToReturn[0] = explorer.getLastEntry().getAdjNode();
            for (int lmIdx = 0; lmIdx < landmarkNodeIdsToReturn.length - 1; lmIdx++) {
                ...
                explorer.runAlgo();
                landmarkNodeIdsToReturn[lmIdx + 1] = explorer.getLastEntry().getAdjNode();
                if (logDetails && lmIdx % logOffset == 0)
                    LOGGER.info("Finding landmarks [{}] in network [{}] for {}. Start node: {} ({}). Progress {}%, {}",
                            lmConfig, explorer.getVisitedNodes(), info, startNode, createPoint(graph, startNode),
                            (int) (100.0 * lmIdx / landmarkNodeIdsToReturn.length), Helper.getMemInfo());
    ...
    private static class LandmarkExplorer extends DijkstraBidirectionRef {
        ...
        protected double calcWeight(EdgeIteratorState iter, SPTEntry currEdge, boolean reverse) {
            if (!accessFilter.accept(iter))
                return Double.POSITIVE_INFINITY;
            return GHUtility.calcWeightWithTurnWeightWithAccess(weighting, iter, reverse, currEdge.getEdge()) + currEdge.getWeightOfVisitedPath();
        }
            ...
            map.forEach((IntObjectPredicate<SPTEntry>) (nodeId, value) -> {
                ...
                if (sn != subnetworkId) {
                    if (sn != UNSET_SUBNETWORK && sn != UNCLEAR_SUBNETWORK) {
                        ...
                        LOGGER.error("subnetworkId for node {} ({}) already set ({}). Cannot change to {}",
                                nodeId, createPoint(graph, nodeId), sn, subnetworkId);
        ...
        public void initLandmarkWeights(final int lmIdx, int lmNodeId, final long rowSize, final int offset) {
            ...
            map.forEach((IntObjectProcedure<SPTEntry>) (nodeId, b) -> {
                if (!lms.setWeight(nodeId * rowSize + lmIdx * 4 + offset, b.getWeight())) {
                    maxedout.incrementAndGet();
                    finalMaxWeight.setValue(Math.max(b.getWeight(), finalMaxWeight.getValue()));
                }
            });

            if ((double) maxedout.get() / map.size() > 0.1) {
                LOGGER.warn("landmark {} ({},{}): too many weights were maxed out ({}/{})" +
                                ". Use a bigger factor than {}. For example, use maximum_lm_weight: {} in your LM profile definition",
                        lmIdx, nodeAccess.getLat(lmNodeId), nodeAccess.getLon(lmNodeId), maxedout.get(), map.size(),
                        lms.factor, finalMaxWeight.getValue() * 1.2);
     ...
     */
    static final Comparator<Map.Entry<Integer, Integer>> SORT_BY_WEIGHT = (o1, o2) -> Integer.compare(o2.getKey(), o1.getKey());
