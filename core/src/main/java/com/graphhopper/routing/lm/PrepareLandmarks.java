...
public class PrepareLandmarks {
    ...
    public void doWork() throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
        if (prepared)
            throw new IllegalStateException("Call doWork only once!");
        prepared = true;
        StopWatch sw = new StopWatch().start();
        LOGGER.info("Start calculating {} landmarks, weighting: {}, {}", lms.getLandmarkCount(), lms.getLmSelectionWeighting(), Helper.getMemInfo());
