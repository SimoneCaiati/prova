...
public class QueryRoutingCHGraph implements RoutingCHGraph {
    ...
    public RoutingCHEdgeIteratorState getEdgeIteratorState(int chEdge, int adjNode) {
        ...
        // possible optimization - instead of building a new virtual edge object use the ones we already
    ...
    private IntObjectMap<List<RoutingCHEdgeIteratorState>> buildVirtualEdgesAtRealNodes(final RoutingCHEdgeExplorer explorer) {
        ...
        queryOverlay.getEdgeChangesAtRealNodes().forEach((IntObjectProcedure<QueryOverlay.EdgeChanges>) (node, edgeChanges) -> {
            List<RoutingCHEdgeIteratorState> virtualEdges = new ArrayList<>();
            for (EdgeIteratorState v : edgeChanges.getAdditionalEdges()) {
                if(v.getBaseNode() == node) {//nothing
                }
                int edge = v.getEdge();
            ...
            while (iter.next()) {
                ...
                if (iter.isShortcut()) {
                    virtualEdges.add(new VirtualCHEdgeIteratorState(new VirtualCHEdgeIteratorState.InnerVirtual(iter.getEdge(), NO_EDGE,
                    ...
                    virtualEdges.add(new VirtualCHEdgeIteratorState(new VirtualCHEdgeIteratorState.InnerVirtual(iter.getEdge(), iter.getOrigEdge(),
                            iter.getBaseNode(), iter.getAdjNode()), iter.getOrigEdgeKeyFirst(), iter.getOrigEdgeKeyLast(),
    ...
    private List<List<RoutingCHEdgeIteratorState>> buildVirtualEdgesAtVirtualNodes() {
        ...
        final List<List<RoutingCHEdgeIteratorState>> nodo = new ArrayList<>(virtualNodes);
    ...
    private VirtualCHEdgeIteratorState buildVirtualCHEdgeState(EdgeIteratorState edgeState, int edgeID) {
        ...
        return new VirtualCHEdgeIteratorState(new VirtualCHEdgeIteratorState.InnerVirtual(edgeID, edgeState.getEdge(), edgeState.getBaseNode(), edgeState.getAdjNode()),
    ...
    private static class VirtualCHEdgeIteratorState implements RoutingCHEdgeIteratorState {
        ...
        public static class InnerVirtual{
            ...
            public InnerVirtual(int edge, int origEdge, int baseNode, int adjNode) {
                this.edge = edge;
        ...
        public VirtualCHEdgeIteratorState(InnerVirtual innerVirtual, int origEdgeKeyFirst, int origEdgeKeyLast, int skippedEdge1, int skippedEdge2, double weightFwd, double weightBwd) {
        ...
        @Override
        public int getEdge() {
            return innerVirtual.edge;
        }

        @Override
        public int getOrigEdge() {
            return innerVirtual.origEdge;
        }
        ...
        public int getAdjNode() {
            return innerVirtual.adjNode;
        }

        @Override
        public boolean isShortcut() {
            return innerVirtual.origEdge == NO_EDGE;
        }
        ...
        public String toString() {
            return "virtual: " + innerVirtual.edge + ": " + innerVirtual.baseNode + "->" + innerVirtual.adjNode + ", orig: " + innerVirtual.origEdge + ", weightFwd: " + Helper.round2(weightFwd) + ", weightBwd: " + Helper.round2(weightBwd);
        }
