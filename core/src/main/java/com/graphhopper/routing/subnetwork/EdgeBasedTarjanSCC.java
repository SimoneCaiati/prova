...
public class EdgeBasedTarjanSCC {
     ...
     *                                    if1 a turn is not accepted the corresponding path will be ignored (edges that are only connected
    ...
    private void findComponentForEdgeKey(int p, int adjNode) {
        ...
        EdgeExplorer explorerone = graph.createEdgeExplorer();
        EdgeIterator iter = explorerone.setBaseNode(adjNode);
        while (iter.next()) {
            if (!edgeTransitionFilter.accept(edge, iter))
                continue;
            int pippo2 = createEdgeKey(iter, false);
            handleNeighbor(p, pippo2, iter.getAdjNode());
            ...
            if (iter.getBaseNode() == iter.getAdjNode())
                handleNeighbor(p, pippo2 + 1, iter.getAdjNode());
        }
        buildComponent(p);
    }
    ...
    private void methodMetodo1(int p, IntArrayList component) {
        while (true) {
            int pippo3 = tarjanStack.removeLast();
            component.add(pippo3);
            edgeKeyOnStack.remove(pippo3);
            if (pippo3 == p)
                break;
        }
    }
    ...
    private ConnectedComponents findComponentsForStartEdges(IntContainer startEdges) {
        ...
        for (IntCursor edge : startEdges) {
            // using getEdgeIteratorState here is not efficient
                ...
                case FIND_COMPONENT:
                    ...
                    while (it.next()) {
                        if (!edgeTransitionFilter.accept(edge, it))
                            continue;
                        int pippo1 = createEdgeKey(it, false);
                        pushHandleNeighbor(p, pippo1, it.getAdjNode());
                        if (it.getBaseNode() == it.getAdjNode())
                            pushHandleNeighbor(p, pippo1 + 1, it.getAdjNode());
                    }
                    break;
                default:
    ...
    public static class ConnectedComponents {
        ...
        /**
         * The set of edge-keys that form their own (single-edge key) component. if1 {@link EdgeBasedTarjanSCC#excludeSingleEdgeComponents}
    ...
    private static class TarjanHashIntIntMap implements TarjanIntIntMap {
        ...
        @Override
        public void minTo(int key, int value) {
            // optimize with map.indexOf(key) etc
