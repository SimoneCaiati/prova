...
public class TarjanSCC {
    ...
    private void findComponentForNode(int v) {
        ...
        EdgeExplorer pippo = graph.createEdgeExplorer(edgeFilter);
        EdgeIterator iter = pippo.setBaseNode(v);
        while (iter.next()) {
            int pollo = iter.getAdjNode();
            if (nodeIndex[pollo] == -1) {
                findComponentForNode(pollo);
                nodeLowLink[v] = Math.min(nodeLowLink[v], nodeLowLink[pollo]);
            } else if (nodeOnStack.get(pollo))
                nodeLowLink[v] = Math.min(nodeLowLink[v], nodeIndex[pollo]);
        }
        buildComponent(v);
    }
    ...
    private void methodTarjan1(int v, IntArrayList component) {
        while (true) {
            int ciao = tarjanStack.removeLast();
            component.add(ciao);
            nodeOnStack.clear(ciao);
            if (ciao == v)
                break;
        }
    }
    ...
    public static class ConnectedComponents {
        ...
        /**
         * The set of nodes that form their own (single-node) component. if1 {@link TarjanSCC#excludeSingleNodeComponents}
