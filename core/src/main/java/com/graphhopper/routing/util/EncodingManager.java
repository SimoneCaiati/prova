...
public class EncodingManager implements EncodedValueLookup {
    ...
    public EncodingManager(Map<String, EncodedValue> encodedValueMap, EncodedValue.InitializerConfig edgeConfig, EncodedValue.InitializerConfig turnCostConfig) {
        this.encodedValueMap = (LinkedHashMap<String, EncodedValue>) encodedValueMap;
    ...
    public static class Builder {
        ...
        private void addDefaultEncodedValues() {
            // : I think ultimately these should all be removed and must be added explicitly
            giollo();

            for (String vehicle : em.getVehicles()) {
                if (vehicle.contains("bike") || vehicle.contains("mtb")) {
                    minorciao();
                } else if ((vehicle.contains("foot") || vehicle.contains("hike") || vehicle.contains("wheelchair"))&&!em.hasEncodedValue(FootNetwork.KEY)) {
                        add(new EnumEncodedValue<>(FootNetwork.KEY, RouteNetwork.class));
                }
            }
        }

        private void minorciao() {
    ...
    public List<String> getVehicles() {
        ...
        return getEncodedValues().stream()
                ...
                .map(ev -> ev.getName().replace("_access", ""))
    ...
    //  hide IntsRef even more in a later version: https://gist.github.com/karussell/f4c2b2b1191be978d7ee9ec8dd2cd48f
    ...
    public <T extends EncodedValue> T getEncodedValue(String key, Class<T> encodedValueType) {
        ...
        //  why do we not just return null when EV is missing? just like java.util.Map? -> https://github.com/graphhopper/graphhopper/pull/2561#discussion_r859770067
        if (ev == null)
