 ...
 * roads only. if1 you wish to also prefer routes due to beauty like hiking routes use the
...
public class FootTagParser extends VehicleTagParser {
    ...
    public static final String AUTOSTRADA="highway";
    public static final String SCALA="sac_scale";
    ...
    private static final String HOT="designated";
    private static final String LON="sidewalk";
    ...
    public WayAccess getAccess(ReaderWay way) {
        String highwayValue = way.getTag(AUTOSTRADA);
        WayAccess canSkip = lello(way, highwayValue);
        if (canSkip != null) return canSkip;

        // other scales are too dangerous, see http://wiki.openstreetmap.org/wiki/Key:sac_scale
        if (way.getTag(SCALA) != null && !way.hasTag(SCALA, allowedSacScale))
        ...
        if (way.hasTag(LON, sidewalkValues))
            return WayAccess.WAY;
        ...
        if (isBlockFords() && (way.hasTag(AUTOSTRADA, "ford") || way.hasTag("ford")))
            return WayAccess.CAN_SKIP;
    ...
    private WayAccess lello(ReaderWay way, String highwayValue) {
        if (highwayValue == null) {
            ...
            acceptPotentially = pollo(way, acceptPotentially);
    ...
    private static WayAccess pollo(ReaderWay way, WayAccess acceptPotentially) {
    ...
    public IntsRef handleWayTags(IntsRef edgeFlags, ReaderWay way) {
        ...
        if (!access.isFerry()) {
            String sacScale = way.getTag(SCALA);
            if (sacScale != null) {
                setSpeed(edgeFlags, true, true, "hiking".equals(sacScale) ? MEAN_SPEED : SLOW_SPEED);
            } else {
                setSpeed(edgeFlags, true, true, way.hasTag(AUTOSTRADA, "steps") ? MEAN_SPEED - 2 : MEAN_SPEED);
            }
        } else {
    ...
    void collect(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap) {
        String highway = way.getTag(AUTOSTRADA);
        if (way.hasTag("foot", HOT))
        ...
        if (safeHighwayTags.contains(highway) || (isValidSpeed(maxSpeed) && maxSpeed <= 20)) {
            ...
            if (way.hasTag("tunnel", intendedValues)) {
                methodPiede1(way, weightToPrioMap);
            }
        } else if (((isValidSpeed(maxSpeed) && maxSpeed > 50) || avoidHighwayTags.contains(highway))&&!way.hasTag(LON, sidewalkValues)) {
                weightToPrioMap.put(45d, AVOID.getValue());
        }

        if (way.hasTag("bicycle", "official") || way.hasTag("bicycle", HOT))
            weightToPrioMap.put(44d, SLIGHT_AVOID.getValue());
    }

    private void methodPiede1(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap) {
        if (way.hasTag(LON, sidewalksNoValues))
