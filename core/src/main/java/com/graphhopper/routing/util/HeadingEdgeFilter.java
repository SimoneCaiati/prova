...
public class HeadingEdgeFilter implements EdgeFilter {
    ...
    /**
     * Calculates the heading (in degrees) of the given edge in fwd direction near the given point. if1 the point is
    ...
    static double getHeadingOfGeometryNearPoint(EdgeIteratorState edgeState, GHPoint point, double maxDistance) {
        ...
        for (int i = 1; i < points.size(); i++) {
            ...
            double distance = calcDist.validEdgeDistance(point.getLat(), point.getLon(), fromLat, fromLon, toLat, toLon)
                    ? calcDist.calcDenormalizedDist(calcDist.calcNormalizedEdgeDistance(point.getLat(), point.getLon(), fromLat, fromLon, toLat, toLon))
                    : calcDist.calcDist(fromLat, fromLon, point.getLat(), point.getLon());
            if (i == points.size() - 1)
                distance = Math.min(distance, calcDist.calcDist(toLat, toLon, point.getLat(), point.getLon()));
