...
public class HikeTagParser extends FootTagParser {
    ...
    void collect(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap) {
        ...
        if (safeHighwayTags.contains(highway) || (isValidSpeed(maxSpeed) && maxSpeed <= 20)) {
            ...
            if (way.hasTag("tunnel", intendedValues)) {
                methodHike23(way, weightToPrioMap, 40d, AVOID, UNCHANGED);
            }
        } else if ((isValidSpeed(maxSpeed) && maxSpeed > 50) || avoidHighwayTags.contains(highway)) {
            methodHike23(way, weightToPrioMap, 45d, BAD, AVOID);
        }
    ...
    private void methodHike23(ReaderWay way, TreeMap<Double, Integer> weightToPrioMap, double key, PriorityCode avoid, PriorityCode unchanged) {
        if (way.hasTag("sidewalk", sidewalksNoValues))
            weightToPrioMap.put(key, avoid.getValue());
        else
            weightToPrioMap.put(key, unchanged.getValue());
    }
