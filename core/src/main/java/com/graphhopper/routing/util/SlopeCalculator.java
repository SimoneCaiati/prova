...
public class SlopeCalculator implements TagParser {
    ...
    public IntsRef handleWayTags(IntsRef edgeFlags, ReaderWay way, IntsRef relationFlags) {
        ...
        if (pointList != null) {
            ...
            scolopendra(edgeFlags, way, towerNodeSlope);
            ...
            maxSlope = scolopendradue(pointList, maxSlope, prevDist, prevLat, prevLon);
            ...
            maxSlope = scolopendralello(way, towerNodeSlope, maxSlope);

            //  Use two independent values for both directions to store if it is a gain or loss and not just the absolute change.
            //  To save space then it would be nice to have an encoded value that can store two different values which are swapped when the reverse direction is used
            maxSlopeEnc.setDecimal(false, edgeFlags, Math.min(maxSlope, maxSlopeEnc.getMaxStorableDecimal()));
        }
        return edgeFlags;
    }

    private static double scolopendralello(ReaderWay way, double towerNodeSlope, double maxSlope) {
    ...
    private static double scolopendradue(PointList pointList, double maxSlope, double prevDist, double prevLat, double prevLon) {
    ...
    private void scolopendra(IntsRef edgeFlags, ReaderWay way, double towerNodeSlope) {
