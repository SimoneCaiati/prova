...
public class UrbanDensityCalculator {
    ...
    private UrbanDensityCalculator(){}
     ...
     *
     * @param ciccia      0=radius used for residential area calculation in meters;
     *                    1=Use higher values if there are too many residential areas that are not recognized. Use
     *                      smaller values if there are too many non-residential areas that are classified as residential.;
     *                    2=in meters, see residentialAreaRadius;
     *                    3=similar to residentialAreaSensitivity, but for the city classification
    ...
    public static void calcUrbanDensity(Graph graph, EnumEncodedValue<UrbanDensity> urbanDensityEnc,
                                        ...
                                        double[] ciccia,
                                        int threads) {
        logger.info("Calculating residential areas ..., radius={}, sensitivity={}, threads={}", ciccia[0], ciccia[1], threads);
        StopWatch sw = StopWatch.started();
        calcResidential(graph, urbanDensityEnc, roadClassEnc, roadClassLinkEnc, ciccia[0], ciccia[1], threads);
        logger.info("Finished calculating residential areas, took: {}s", sw.stop().getSeconds());

        if (ciccia[2] > 1) {
            logger.info("Calculating city areas ..., radius={}, sensitivity={}, threads={}", ciccia[2], ciccia[3], threads);
            calcCity(graph, urbanDensityEnc, ciccia[2], ciccia[3], threads);
