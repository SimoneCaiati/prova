...
public class FastestWeighting extends AbstractWeighting {
    public static String getRoadAccessDestinationFactor() {
        return roadAccessDestinationFactor;
    }

    protected static String roadAccessDestinationFactor = "road_access_destination_factor";

    public static String getAccessPrivateFactor() {
        return accessPrivateFactor;
    }

    protected static String accessPrivateFactor = "road_access_private_factor";
    /**
    ...
    public FastestWeighting(BooleanEncodedValue accessEnc, DecimalEncodedValue speedEnc, EnumEncodedValue<RoadAccess> roadAccessEnc, PMap map, TurnCostProvider turnCostProvider) {
        ...
        destinationPenalty = map.getDouble(roadAccessDestinationFactor, 1);
        privatePenalty = map.getDouble(accessPrivateFactor, 1);
        // ensure that we do not need to change getMinWeight, i.e. both factors need to be >= 1
        checkBounds(roadAccessDestinationFactor, destinationPenalty, 1, 10);
        checkBounds(accessPrivateFactor, privatePenalty, 1, 10);
