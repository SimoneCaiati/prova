...
class ConditionalExpressionVisitor implements Visitor.AtomVisitor<Boolean, Exception> {
    ...
    public Boolean visitRvalue(Java.Rvalue rv) throws Exception {
        ...
        if (rv instanceof Java.Literal) {
            ...
            if (binOp.lhs instanceof Java.AmbiguousName && ((Java.AmbiguousName) binOp.lhs).identifiers.length == 1) {
                ...
                serio(binOp, startRH, lhVarAsString, eqOps);
            }
    ...
    private Boolean strunz(Java.Rvalue rv, String arg) {
    ...
    private Boolean maiolino(Java.UnaryOperation rv) throws Exception {
        Java.UnaryOperation uo = rv;
        if (uo.operator.equals("!")) return uo.operand.accept(this);
        if (uo.operator.equals("-")) return uo.operand.accept(this);
        return false;
    }

    private boolean lello(Java.MethodInvocation mi) {
    ...
    private void serio(Java.BinaryOperation binOp, int startRH, String lhVarAsString, boolean eqOps) {
        if (binOp.rhs instanceof Java.StringLiteral) {
            ...
            ciaooo(binOp, startRH, lhVarAsString, eqOps, rhValueAsString);
        }
    }

    private void ciaooo(Java.BinaryOperation binOp, int startRH, String lhVarAsString, boolean eqOps, String rhValueAsString) {
