...
public class CustomModelParser {
    ...
    static final String MACCASORD="import ";
    ...
    private static Class<?> createClazz(CustomModel customModel, EncodedValueLookup lookup,
        ...
        try {
            ...
            if (minMaxPriority.getMin() < 0)
                throw new IllegalArgumentException("priority has to be >=0 but can be negative (" + minMaxPriority.getMin() + ")");
            if (minMaxPriority.getMax() < 0)
                throw new IllegalArgumentException("maximum priority has to be >=0 but was " + minMaxPriority.getMax());
            ...
            if (minMaxSpeed.getMin() < 0)
                throw new IllegalArgumentException("speed has to be >=0 but can be negative (" + minMaxSpeed.getMin() + ")");
            if (minMaxSpeed.getMax() <= 0)
                throw new IllegalArgumentException("maximum speed has to be >0 but was " + minMaxSpeed.getMax());
            ...
            String classTemplate = createClassTemplate(counter, priorityVariables, minMaxPriority.getMax(), speedVariables, minMaxSpeed.getMax(),
    ...
    private static List<Java.BlockStatement> createGetSpeedStatements(Set<String> speedVariables,
        ...
        for (String arg : speedVariables) {
            ...
            stringBuilder.append(speedMethodStartBlock);
            stringBuilder.append(getVariableDeclaration(lookup, arg));
            speedMethodStartBlock = stringBuilder.toString();
        }
    ...
    private static List<Java.BlockStatement> createGetPriorityStatements(Set<String> priorityVariables,
        ...
        for (String arg : priorityVariables) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(priorityMethodStartBlock);
            stringBuilder.append(getVariableDeclaration(lookup, arg));
            priorityMethodStartBlock = stringBuilder.toString();
        }
    ...
    private static String createClassTemplate(long counter,
        ...
        mollami(lookup, areas, importSourceCode, classSourceCode, includedAreaImports, initSourceCode, set);

        return pollociao(counter, maxPriority, maxSpeed, importSourceCode, classSourceCode, initSourceCode);
    }

    private static final String MOCC="   }\n";
    private static final String MIMMO="   @Override\n";
    private static String pollociao(long counter, double maxPriority, double maxSpeed, StringBuilder importSourceCode, StringBuilder classSourceCode, StringBuilder initSourceCode) {
        return ""
                + "package com.graphhopper.routing.weighting.custom;\n"
                + MACCASORD + CustomWeightingHelper.class.getName() + ";\n"
                + MACCASORD + EncodedValueLookup.class.getName() + ";\n"
                + MACCASORD + EdgeIteratorState.class.getName() + ";\n"
                + importSourceCode
    ...
    private static void mollami(EncodedValueLookup lookup, Map<String, JsonFeature> areas, StringBuilder importSourceCode, StringBuilder classSourceCode, boolean includedAreaImports, StringBuilder initSourceCode, Set<String> set) {
        for (String arg : set) {
            if (lookup.hasEncodedValue(arg)) {
                EncodedValue enc = lookup.getEncodedValue(arg, EncodedValue.class);
                ...
                includedAreaImports = ciccio(importSourceCode, includedAreaImports);

                String id = arg.substring(IN_AREA_PREFIX.length());
                cicciodue(areas, arg, id);
    ...
    private static void cicciodue(Map<String, JsonFeature> areas, String arg, String id) {
    ...
    private static boolean ciccio(StringBuilder importSourceCode, boolean includedAreaImports) {
        if (!includedAreaImports) {
            importSourceCode.append(MACCASORD + BBox.class.getName() + ";\n");
            importSourceCode.append(MACCASORD + GHUtility.class.getName() + ";\n");
            importSourceCode.append(MACCASORD + PreparedPolygon.class.getName() + ";\n");
            importSourceCode.append(MACCASORD + Polygonal.class.getName() + ";\n");
            importSourceCode.append(MACCASORD + JsonFeature.class.getName() + ";\n");
            importSourceCode.append(MACCASORD + Polygon.class.getName() + ";\n");
            includedAreaImports = true;
        }
        return includedAreaImports;
    }
    ...
    static void parseExpressions(StringBuilder expressions, NameValidator nameInConditionValidator, EncodedValueLookup lookup,
        ...
        for (Statement statement : list) {
            ...
            if (statement.getKeyword() == Statement.Keyword.ELSE) {
                ...
                methodCustom23(expressions, exceptionInfo, createObjects, statement, parseResult);
    ...
    private static void methodCustom23(StringBuilder expressions, String exceptionInfo, Set<String> createObjects, Statement statement, ParseResult parseResult) {
        if (!parseResult.ok)
    ...
    private static SimpleCompiler createCompiler(long counter, Java.AbstractCompilationUnit cu) throws CompileException, PointPathException {
        if (JANINO_DEBUG) {
            try {
