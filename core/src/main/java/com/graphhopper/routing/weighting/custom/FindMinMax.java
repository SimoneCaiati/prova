    ...
    private static void checkMultiplyValue(List<Statement> list, EncodedValueLookup lookup) {
        ...
        for (Statement statement : list) {
            if (statement.getOperation() == Statement.Op.MULTIPLY) {
                ...
                if (minMax.getMax() > 1)
                    throw new IllegalArgumentException("maximum of value '" + statement.getValue() + "' cannot be larger than 1, but was: " + minMax.getMax());
                else if (minMax.getMin() < 0)
                    throw new IllegalArgumentException("minimum of value '" + statement.getValue() + "' cannot be smaller than 0, but was: " + minMax.getMin());
            }
        }
    }
    ...
    private static void findMinMaxForBlock(Set<String> createdObjects, final MinMax minMax, List<Statement> block, EncodedValueLookup lookup) {
        ...
        if (block.get(0).getCondition().trim().equals("true")) {
            ...
            for (Statement s : block) {
                ...
                minMaxBlock.setMin(Math.min(minMaxBlock.getMin(), tmp.getMin()));
                minMaxBlock.setMax(Math.max(minMaxBlock.getMax(), tmp.getMax()));
            }

            // if there is no 'else' statement it's like there is a 'neutral' branch that leaves the initial value as is
            if (!foundElse) {
                minMaxBlock.setMin(Math.min(minMaxBlock.getMin(), minMax.getMin()));
                minMaxBlock.setMax(Math.max(minMaxBlock.getMax(), minMax.getMax()));
            }
        }

        minMax.setMin(minMaxBlock.getMin());
        minMax.setMax(minMaxBlock.getMax());
    }
