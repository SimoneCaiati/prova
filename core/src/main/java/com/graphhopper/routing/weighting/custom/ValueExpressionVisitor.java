...
public class ValueExpressionVisitor implements Visitor.AtomVisitor<Boolean, Exception> {
    ...
    public Boolean visitRvalue(Java.Rvalue rv) throws Exception {
        ...
        if (rv instanceof Java.Literal) {
            ...
            if (sen(binOp, op)) return binOp.lhs.accept(this) && binOp.rhs.accept(this);
    ...
    private Boolean gev(String arg) {
        if (isValidIdentifier(arg)) return true;
        invalidMessage = "'" + arg + "' not available";
        return false;
    }

    private boolean sen(Java.BinaryOperation binOp, String op) {
        return (op.equals("*") || op.equals("+") || binOp.operator.equals("-"));
    }


    private Boolean kola(Java.UnaryOperation rv) throws Exception {
        Java.UnaryOperation uop = rv;
    ...
    private boolean gol(Java.MethodInvocation mi) {
    ...
    static MinMax findMinMax(Set<String> createdObjects, String valueExpression, EncodedValueLookup lookup) {
        ...
        try {
            ...
            ExpressionEvaluator ee = new ExpressionEvaluator();
            String s = result.guessedVariables.iterator().next();
            ee.setParameters(new String[]{s}, new Class[]{double.class});
            ee.cook(valueExpression);
            double max = getMax(lookup.getEncodedValue(s, EncodedValue.class));
            Number val1 = (Number) ee.evaluate(max);
            double min = getMin(lookup.getEncodedValue(s, EncodedValue.class));
