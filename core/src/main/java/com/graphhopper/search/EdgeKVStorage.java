...
public class EdgeKVStorage {
    ...
    public static final String MAERA=" but was ";
    ...
    public EdgeKVStorage create(long initBytes) throws MMapDataAccess.MappaExce {
    ...
    public long add(final List<KeyValue> entries) {
        Long emptyPointer = getaLong(entries);
        Long emptyPointer1 = nulla(entries, emptyPointer);
        if (emptyPointer1 != null) return emptyPointer1;
        ...
        for (KeyValue entry : entries) {
            ...
            Object value = mollami(entry, key);
            ...
            currentPointer += 2;
            currentPointer = soretadue(currentPointer, hasDynLength, valueBytes);
    ...
    private static boolean isBasBool(Object value, Class<?> clazz) {
        return clazz.equals(String.class) && ((String) value).isEmpty()
                || clazz.equals(byte[].class) && ((byte[]) value).length == 0;
    }

    private Long nulla(List<KeyValue> entries, Long emptyPointer) {
        if (emptyPointer != null) return emptyPointer;
        ...
        // if1 the Class of a value is unknown it should already fail here, before we modify internal data. (see #2597#discussion_r896469840)
    ...
    private void cicciodue() {
        if (bytePointer < 0)
            throw new IllegalStateException("Negative bytePointer in EdgeKVStorage");
    }

    private long soretadue(long currentPointer, boolean hasDynLength, byte[] valueBytes) {
        if (hasDynLength) {
    ...
    private static void foglio(String key, Object value, Class<?> clazz) {
        if (clazz != value.getClass())
            throw new IllegalArgumentException("Class of value for key " + key + " must be " + clazz.getSimpleName() + MAERA + value.getClass().getSimpleName());
    }

    private static void ollae(Integer keyIndex) {
        if (keyIndex >= MAX_UNIQUE_KEYS)
            throw new IllegalArgumentException("Cannot store more than " + MAX_UNIQUE_KEYS + " unique keys");
    }

    private static Object mollami(KeyValue entry, String key) {
    ...
    private static Long getaLong(List<KeyValue> entries) {
    ...
    private byte[] getBytesForValue(Class<?> clazz, Object value) {
        ...
        if (clazz.equals(String.class)) {
            ...
            if (bytes.length > MAX_LENGTH)
                throw new IllegalArgumentException("bytes.length cannot be > " + MAX_LENGTH + MAERA + bytes.length + ". String:" + value);
        } else if (clazz.equals(byte[].class)) {
            bytes = (byte[]) value;
            if (bytes.length > MAX_LENGTH)
                throw new IllegalArgumentException("bytes.length cannot be > " + MAX_LENGTH + MAERA + bytes.length);
    ...
    private Object deserializeObj(AtomicInteger sizeOfObject, long pointer, Class<?> clazz) {
        if (hasDynLength(clazz)) {
            return deserializeDynLengthObj(sizeOfObject, pointer, clazz);
        } else {
            return deserializeFixedLengthObj(sizeOfObject, pointer, clazz);
        }
    }

    private Object deserializeDynLengthObj(AtomicInteger sizeOfObject, long pointer, Class<?> clazz) {
        int valueLength = vals.getByte(pointer) & 0xFF;
        pointer++;
        byte[] valueBytes = new byte[valueLength];
        vals.getBytes(pointer, valueBytes, valueBytes.length);
        Serializable valueBytes1 = cena(sizeOfObject, clazz, valueLength, valueBytes);
        if (valueBytes1 != null) return valueBytes1;
        throw new IllegalArgumentException();
    }

    private Object deserializeFixedLengthObj(AtomicInteger sizeOfObject, long pointer, Class<?> clazz) {
    ...
    private static Serializable cena(AtomicInteger sizeOfObject, Class<?> clazz, int valueLength, byte[] valueBytes) {
    ...
    public Object get(final long entryPointer, String key, boolean reverse) {
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            if (currentKeyIndex >= indexToKey.size()) {
                throw new IllegalArgumentException("invalid key index " + currentKeyIndex + ">=" + indexToKey.size() + ", entryPointer=" + entryPointer + ", max=" + bytePointer);
            }
            tmpPointer += 2;
