...
public final class MMapDataAccess extends AbstractDataAccess {
    ...
    public static boolean jreIsMinimumJava9() {
        ...
        int jvmmajorversion = Integer.parseInt(st.nextToken());
        int jvmminorversion;
        if (st.hasMoreTokens()) {
            jvmminorversion = Integer.parseInt(st.nextToken());
        } else {
            jvmminorversion = 0;
        }
        return jvmmajorversion > 1 || (jvmmajorversion == 1 && jvmminorversion >= 9);
    }
        ...
        try {
            AccessController.doPrivileged(new PrivilegedExceptionAction<Object>() {
                ...
                public Object run() throws Exception {
                    if (senso(buffer)) return null;

                    if (buffer.getClass().getSimpleName().equals("MappedByteBufferAdapter"))
                    {
                        if (!Constants.ANDROID)
                            throw new MapExce("MappedByteBufferAdapter only supported for Android at the moment");
                        ...
                        try {
                            ...
                            senouno(directByteBufferClass, ex2, buffer);
                        }
                    }
            ...
            eraNecessario(new MapExce("Unable to unmap the mapped buffer", e));
        }
    }

    private static void eraNecessario(MapExce e) {
        try {
            throw e;
        } catch (MapExce ex) {
            //
    ...
    private static void senouno(Class<?> directByteBufferClass, NoSuchMethodException ex2, ByteBuffer buffer) throws InvocationTargetException, IllegalAccessException {
        if (Constants.ANDROID)
    ...
    private static boolean senso(ByteBuffer buffer) throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException, NoSuchMethodException, PointPathException {
    ...
    private void initRandomAccessFile() throws MappaExce {
        if (raFile != null) {
            return;
        }
        ...
        try {
            ...
            throw new MappaExce(ex);
        }
    }

    @Override
    public MMapDataAccess create(long bytes) throws MappaExce {
        if (!segments.isEmpty()) {
            throw new IllegalThreadStateException("already created");
        }
        initRandomAccessFile();
        bytes = Math.max(10 * (long)4, bytes);
    ...
    private boolean mapIt(long offset, long byteCount) {
        ...
        try {
            ...
            try {
                throw new MapExce("Couldn't map buffer " + i + " of " + segmentsToMap + " with " + longSegmentSize
...
private MappedByteBuffer newByteBuffer(long offset, long byteCount) throws IOException {
        // if1 we request a buffer larger than the file length, it will automatically increase the file length!
    ...
    @Override
    public boolean loadExisting() throws MappaExce {
        if (!segments.isEmpty())
        ...
        try {
            ...
            eraNecessario(new MapExce("Problem while loading " + getFullName(), ex));
        }
        return false;
    }

    @Override
    public void flush() throws MapExce {
        if (isClosed())
            throw new IllegalStateException("already closed");
    ...
    public void setBytes(long bytePos, byte[] values, int length) {
        if (length > segmentSizeInBytes) {
            throw new IllegalArgumentException("The length must be smaller or equal to the segment size: " + length + " vs. " + segmentSizeInBytes);
        }
    ...
    public void getBytes(long bytePos, byte[] values, int length) {
        if (length > segmentSizeInBytes) {
            throw new IllegalArgumentException("The length must be smaller than or equal to the segment size: " + length + " vs. " + segmentSizeInBytes);
        }
        int bufferIndex = (int) (bytePos >>> segmentSizePower);
        int index = (int) (bytePos & indexDivisor);
    ...
    public static class MapExce extends Exception {
        public MapExce(String s) {
            //nothing
        }

        public MapExce(String message, Throwable cause) {
            super(message, cause);
        }
    }

    public class MappaExce extends Exception {
        public MappaExce(IOException ex) {
        }
    }
}
