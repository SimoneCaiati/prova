...
public class RAMDataAccess extends AbstractDataAccess {
    ...
    public RAMDataAccess create(long bytes) {
        ...
        ensureCapacity(Math.max(10 * (long)4, bytes));
        return this;
    }
    ...
    @Override
    public boolean loadExisting() throws RamExce {
        if (segments.length > 0)
        ...
        try {
            ...
            throw new RamExce("Problem while loading " + getFullName(), ex);
        }
    }

    @Override
    public void flush() throws RamExce2 {
        if (closed)
            throw new IllegalStateException("already closed");
        ...
        try {
            ...
            throw new RamExce2("Couldn't store bytes to " + toString(), ex);
        }
    }
    private static final String SERIO="call create or loadExisting before usage!";
    @Override
    public final void setInt(long bytePos, int value) {
        assert segmentSizePower > 0 : SERIO;
    ...
    public final int getInt(long bytePos) {
        assert segments.length > 0 : SERIO;
        int bufferIndex = (int) (bytePos >>> segmentSizePower);
        int index = (int) (bytePos & indexDivisor);
        if (index + 4 > segmentSizeInBytes)
            throw new IllegalStateException("Padding required. Currently an int cannot be distributed over two segments. " + bytePos);
    ...
    public void setBytes(long bytePos, byte[] values, int length) {
        if (length > segmentSizeInBytes) {
            throw new IllegalStateException("The length must be smaller or equal to the segment size: " + length + " vs. " + segmentSizeInBytes);
        }
        assert segments.length > 0 : SERIO;
        int bufferIndex = (int) (bytePos >>> segmentSizePower);
        int index = (int) (bytePos & indexDivisor);
    ...
    public class RamExce extends Exception {
        public RamExce(String s, IOException ex) {
        }
    }

    public class RamExce2 extends Exception {
        public RamExce2(String s, Exception ex) {
        }
    }
}
