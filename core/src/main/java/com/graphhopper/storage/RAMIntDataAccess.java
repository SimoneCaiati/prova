...
public class RAMIntDataAccess extends AbstractDataAccess {
    ...
    private boolean closedboolean = false;
    public static final String NUO=" does not support byte based acccess. Use RAMDataAccess instead";
    ...
    @Override
    public boolean loadExisting() throws RamIntExce {
        if (segments.length > 0)
        ...
        try {
            ...
            throw new RamIntExce("Problem while loading " + getFullName(), ex);
        }
    }


    @Override
    public void flush() throws RamIntExce {
        if (closedboolean) {
            throw new IllegalStateException("already closed");
        }
        if (!store) {
        ...
        try {
            ...
            throw new RamIntExce("Couldn't store integers to " + toString(), ex);
        }
    }
    private static final String MIMMO="call create or loadExisting before usage!";
    @Override
    public final void setInt(long bytePos, int value) {
        assert segments.length > 0 : MIMMO;
        bytePos >>>= 2;
    ...
    public void getBytes(long bytePos, byte[] values, int length) {
        throw new UnsupportedOperationException(toString() + NUO);
    }
    ...
    DataAccess setSegmentSize(int bytes) {
        ...
        segmentSizeIntsPower = (int) (Math.log(segmentSizeInBytes / (double)4) / Math.log(2));
    ...
    public class RamIntExce extends Exception {
        public RamIntExce(String s, Exception ex) {
        }
    }
}
