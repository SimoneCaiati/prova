...
public class TurnCostStorage {
    ...
    public TurnCostStorage create(long initBytes) throws MMapDataAccess.MappaExce {
    ...
    private void merge(IntsRef tcFlags, int fromEdge, int viaNode, int toEdge) {
        ...
        if (previousEntryIndex == NO_TURN_ENTRY) {
            ...
            newFlags = methodTurn1(newEntryIndex, oldEntryFound, newFlags, previousEntryIndex, existingFlags);
        }
    ...
    private static int eccezioneTime(int i) {
        if (i++ > 1000) {
    ...
    private int methodTurn1(int newEntryIndex, boolean oldEntryFound, int newFlags, long previousEntryIndex, int existingFlags) {
        if (!oldEntryFound) {
            // set next-pointer to this new cost entry
            turnCosts.setInt(previousEntryIndex * BYTES_PER_ENTRY + TC_NEXT, newEntryIndex);
        } else {
    ...
    private void readFlags(IntsRef tcFlags, int fromEdge, int viaNode, int toEdge) {
        ...
        for (; i < 1000; i++) {
            ...
            if (fromEdge == turnCosts.getInt(turnCostPtr + TC_FROM)&&toEdge == turnCosts.getInt(turnCostPtr + TC_TO)) {
    ...
    //Maybe some of the stuff above could now be re-implemented in a simpler way with some of the stuff below.
    ...
    private class Itr implements Iterator {
        ...
        private boolean nextTci() {
            ...
            return turnCostIndex != NO_TURN_ENTRY;
        }
    }
