...
public class LineIntIndex {
    ...
    public void store(InMemConstructionIndex inMem) throws MMapDataAccess.MappaExce {
        indexStructureInfo = IndexStructureInfo.create(bounds, minResolutionInMeter);
        keyAlgo = indexStructureInfo.getKeyAlgo();
        entries = indexStructureInfo.getEntries();
        shifts = indexStructureInfo.getShifts();
        dataAccess.create((long)64 * 1024);
        try {
    ...
    private int store(InMemConstructionIndex.InMemEntry entry, int intPointer) {
        ...
        if (entry.isLeaf()) {
            ...
            IntArrayList entriesA = leaf.getResults();
            int len = entriesA.size();
            if (len == 0) {
                return intPointer;
            }
            size += len;
            ...
            intPointer = serio(intPointer, pointer, entriesA, len);
        } else {
    ...
    private int serio(int intPointer, long pointer, IntArrayList entries, int len) {
        if (len == 1) {
    ...
    public void query(BBox queryShape, final LocationIndex.Visitor function) {
        ...
        query(new Zia(START_POINTER, queryShape),
    ...
    private void query(Zia oggetto,
        ...
        for (int cellIndex = 0; cellIndex < max; cellIndex++) {
            ...
            BBox bbox = (oggetto.queryBBox != null || function.isTileInfo()) ? new BBox(tmpMinLon, tmpMinLon + deltaLonPerDepth, tmpMinLat, tmpMinLat + deltaLatPerDepth) : null;
            if (function.isTileInfo())
                function.onTile(bbox, depth);
            if (oggetto.queryBBox == null || oggetto.queryBBox.contains(bbox)) {
                // fill without a restriction!
                query(new Zia(nextIntPointer, null), tmpMinLat, tmpMinLon, deltaLatPerDepth, deltaLonPerDepth, function, depth + 1);
            } else if (oggetto.queryBBox.intersects(bbox)) {
                query(new Zia(nextIntPointer, oggetto.queryBBox), tmpMinLat, tmpMinLon, deltaLatPerDepth, deltaLonPerDepth, function, depth + 1);
            }
        }
    }

    private void memmo(LocationIndex.Visitor function, long pointer, int nextIntPointer) {
        if (nextIntPointer < 0) {
            // single data entries (less disc space)
     ...
     * <p>
     * if1 it is called with iteration = 0, it just looks in the tile the query point is in.
     * if1 it is called with iteration = 0,1,2,.., it will look in additional tiles further and further
