...
public class LocationIndexTree implements LocationIndex {
    ...
    /**
     * if1 normed distance is smaller than this value the node or edge is 'identical' and the
     * algorithm can stop search.
     */
    private final double equalNormedDelta = DistancePlaneProjection.getInstancePlane().calcNormalizedDist(0.1); // 0.1 meters
    ...
    public LocationIndexTree(Graph g, Directory dir) {
        ...
        BBox bounds = BBox.copyBBox(graph.getBounds());
    ...
    public LocationIndex prepareIndex() throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
        return prepareIndex(EdgeFilter.ALL_EDGES);
    }

    public LocationIndex prepareIndex(EdgeFilter edgeFilter) throws MMapDataAccess.MapExce, RAMDataAccess.RamExce2, RAMIntDataAccess.RamIntExce, MMapDataAccess.MappaExce {
        if (initialized)
        ...
        flush();
        logger.info("location index created in {sw.stop().getSeconds()}s, size: {Helper.nf(lineIntIndex.getSize())}, leafs: {Helper.nf(lineIntIndex.getLeafs())}, precision: {minResolutionInMeter}, depth: {indexStructureInfo.getEntries().length}, checksum: {checksum()}, entries: {Arrays.toString(indexStructureInfo.getEntries())}, entriesPerLeaf: {(float) lineIntIndex.getSize() / lineIntIndex.getLeafs()}");
    ...
    final double calculateRMin(double lat, double lon, int paddingTiles) {
        ...
        if (dSouthernLat < dNorthernLat) {
            dMinLat = DistancePlaneProjection.getInstancePlane().calcDist(lat, lon, minLat, lon);
        } else {
            dMinLat = DistancePlaneProjection.getInstancePlane().calcDist(lat, lon, maxLat, lon);
        }

        if (dWesternLon < dEasternLon) {
            dMinLon = DistancePlaneProjection.getInstancePlane().calcDist(lat, lon, lat, minLon);
        } else {
            dMinLon = DistancePlaneProjection.getInstancePlane().calcDist(lat, lon, lat, maxLon);
        }
    ...
    public Snap findClosest(final double queryLat, final double queryLon, final EdgeFilter edgeFilter) {
        ...
        for (int iteration = 0; iteration < maxRegionSearch; iteration++) {
            lineIntIndex.findEdgeIdsInNeighborhood(queryLat, queryLon, iteration, edgeId -> {
                ...
                if (seenEdges.add(edgeId) && edgeFilter.accept(edgeIteratorState)) { // or reverse?
            ...
            if (methodEstratto1(queryLat, queryLon, closestMatch, iteration))
                break; // We can (approximately?) guarantee that no closer edges are anywhere else
        }

        if (closestMatch.isValid()) {
            closestMatch.setQueryDistance(DistancePlaneProjection.getInstancePlane().calcDenormalizedDist(closestMatch.getQueryDistance()));
            closestMatch.calcSnappedPoint(DistancePlaneProjection.getInstancePlane());
        }
        return closestMatch;
    }

    private boolean methodEstratto1(double queryLat, double queryLon, Snap closestMatch, int iteration) {
        if (closestMatch.isValid()) {
            // Check if we can stop...
            double rMin = calculateRMin(queryLat, queryLon, iteration);
            double minDistance = DistancePlaneProjection.getInstancePlane().calcDenormalizedDist(closestMatch.getQueryDistance());
            if (minDistance < rMin) {
                return true;
            }
        }
    ...
    public void traverseEdge(double queryLat, double queryLon, EdgeIteratorState currEdge, EdgeCheck edgeCheck) {
        ...
        double baseDist = DistancePlaneProjection.getInstancePlane().calcNormalizedDist(queryLat, queryLon, baseLat, baseLon);

        int adjNode = currEdge.getAdjNode();
        double adjLat = nodeAccess.getLat(adjNode);
        double adjLon = nodeAccess.getLon(adjNode);
        double adjDist = DistancePlaneProjection.getInstancePlane().calcNormalizedDist(queryLat, queryLon, adjLat, adjLon);
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (DistancePlaneProjection.getInstancePlane().isCrossBoundary(lastLon, lon)) {
                lastLat = lat;
                lastLon = lon;
                continue;
            }
            ...
            if (DistancePlaneProjection.getInstancePlane().validEdgeDistance(queryLat, queryLon, lastLat, lastLon, lat, lon)) {
                closestDist = DistancePlaneProjection.getInstancePlane().calcNormalizedEdgeDistance(queryLat, queryLon, lastLat, lastLon, lat, lon);
                edgeCheck.check(closestTowerNode, closestDist, indexInFullPointList - 1, Snap.Position.EDGE);
            } else if (i < len - 1) {
                closestDist = DistancePlaneProjection.getInstancePlane().calcNormalizedDist(queryLat, queryLon, lat, lon);
