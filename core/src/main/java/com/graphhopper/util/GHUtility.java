...
public class GHUtility {
    ...
    public static List<String> getProblems(Graph g) {
        ...
        try {
            ...
            for (; nodeIndex < nodes; nodeIndex++) {
                ...
                while (iter.next()) {
                    methodGh23(problems, nodes, nodeIndex, iter);
            ...
            try {
                throw new GraphHopper.LockOffException("problem with node " + nodeIndex, ex);
    ...
    private static void methodGh23(List<String> problems, int nodes, int nodeIndex, EdgeIterator iter) {
    ...
    public static class Salmo{
        ...
        private boolean allowZeroDistance;

        public Salmo(Graph graph, Random random, int numNodes, double meanDegree,boolean allowLoops,boolean allowZeroDistance) {
            this.graph = graph;
            this.random = random;
            this.numNodes = numNodes;
            this.meanDegree = meanDegree;
            this.allowLoops=allowLoops;
            this.allowZeroDistance=allowZeroDistance;
        }
    }
    /**
     * @param speed if null a random speed will be assign to every edge
     */
    public static void buildRandomGraph(Salmo salmo, BooleanEncodedValue accessEnc, DecimalEncodedValue speedEnc, Double speed,
                                        double pNonZeroLoop, double pBothDir, double pRandomDistanceOffset) {
        kola(salmo.graph, salmo.random, salmo.numNodes, salmo.meanDegree);
        double minDist = Double.MAX_VALUE;
        double maxDist = Double.MIN_VALUE;
        int totalNumEdges = (int) (0.5 * salmo.meanDegree * salmo.numNodes);
        int numEdges = 0;
        while (numEdges < totalNumEdges) {
            int from = salmo.random.nextInt(salmo.numNodes);
            int to = salmo.random.nextInt(salmo.numNodes);
            Double distance = kolauno(salmo.graph, salmo.random, salmo.allowLoops, salmo.allowZeroDistance, pNonZeroLoop, from, to);
            if (distance == null) continue;
            // add some random offset, but also allow duplicate edges with same weight
            if (salmo.random.nextDouble() < pRandomDistanceOffset)
                distance += salmo.random.nextDouble() * distance * 0.01;
            minDist = Math.min(minDist, distance);
            maxDist = Math.max(maxDist, distance);
            // using bidirectional edges will increase mean degree of graph above given value
            boolean bothDirections = salmo.random.nextDouble() < pBothDir;
            EdgeIteratorState edge = salmo.graph.edge(from, to).setDistance(distance).set(accessEnc, true);
            if (bothDirections) edge.setReverse(accessEnc, true);
            double fwdSpeed = 10 + salmo.random.nextDouble() * 110;
            double bwdSpeed = 10 + salmo.random.nextDouble() * 110;
    ...
    private static Double kolauno(Graph graph, Random random, boolean allowLoops, boolean allowZeroDistance, double pNonZeroLoop, int from, int to) {
    ...
    private static void kola(Graph graph, Random random, int numNodes, double meanDegree) {
    ...
    public static double getDistance(int from, int to, NodeAccess nodeAccess) {
        ...
        return DistancePlaneProjection.getInstancePlane().calcDist(fromLat, fromLon, toLat, toLon);
    }
        ...
        for (int node = 0; node < graph.getNodes(); ++node) {
            if (random.nextDouble() < pNodeHasTurnCosts) {
                ...
                while (inIter.next()) {
                    ...
                    double[] mimmo=new double[2];
                    mimmo[0]=pEdgePairHasTurnCosts;
                    mimmo[1]=pCostIsRestriction;
                    int[] mario=new int[2];
                    mario[0]=maxTurnCost;
                    mario[1]=node;
                    jona(turnCostEnc,mario , turnCostStorage, random, mimmo, inIter, outIter);
                }
            }
        }
    }

    private static void jona(DecimalEncodedValue turnCostEnc, int[] mario, TurnCostStorage turnCostStorage, Random random, double[] mimmo, EdgeIterator inIter, EdgeIterator outIter) {
        while (outIter.next()) {
            if (inIter.getEdge() == outIter.getEdge()) {
                // leave u-turns as they are
                continue;
            }
            if (random.nextDouble() < mimmo[0]) {
                double cost = random.nextDouble() < mimmo[1] ? Double.POSITIVE_INFINITY : random.nextDouble() * mario[0];
                turnCostStorage.set(turnCostEnc, inIter.getEdge(), mario[1], outIter.getEdge(), cost);
            }
        }
    }
     ...
     */
    public static BaseGraph newGraph(BaseGraph baseGraph) throws MMapDataAccess.MappaExce {
    ...
    public static EdgeIteratorState createMockedEdgeIteratorState(final double distance, final IntsRef flags) {
        return createMockedEdgeIteratorState(distance, flags, 0, 1, 2);
    }

    public static EdgeIteratorState createMockedEdgeIteratorState(final double distance, final IntsRef flags,
                                                                  final int base, final int adj, final int edge) {
    ...
    public static long calcMillisWithTurnMillis(Weighting weighting, EdgeIteratorState edgeState, boolean reverse, int prevOrNextEdgeId) {
        ...
        // why no first/last orig edge here as in calcWeight ?
    ...
    public static List<CustomArea> readCountries() {
        ...
        try (Reader reader = new InputStreamReader(GHUtility.class.getResourceAsStream("/com/graphhopper/countries/countries.geojson"), StandardCharsets.UTF_8)) {
            ...
            return jsonFeatureCollection.getFeatures().stream()
                    ...
                    .filter(customArea -> map.get(customArea.getProperties().get("id")) != null)
                    .map(f -> {
                        CustomArea ca = CustomArea.fromJsonFeature(f);
                        Country country = map.get(f.getProperties().get("id"));
