...
public class PathMerger {
    ...
    protected static final String TESTA="heading";
    ...
    public ResponsePath doWork(PointList waypoints, List<Path> paths, EncodedValueLookup evLookup, Translation tr) {
        ...
        for (int pathIndex = 0; pathIndex < paths.size(); pathIndex++) {
            ...
            lello(paths, evLookup, tr, fullInstructions, pathIndex, path);
            if (calcPoints || enableInstructions) {
                PointList tmpPoints = path.calcPoints();
                fullPoints = lellouno(paths, fullPoints, pathIndex, tmpPoints);
    ...
    private static PointList lellouno(List<Path> paths, PointList fullPoints, int pathIndex, PointList tmpPoints) {
        if (fullPoints.isEmpty())
    ...
    private void lello(List<Path> paths, EncodedValueLookup evLookup, Translation tr, InstructionList fullInstructions, int pathIndex, Path path) {
        if (enableInstructions) {
     ...
     * if1 the requests contains a heading, this method can transform the first continue to a u-turn if the heading
    ...
    private InstructionList updateInstructionsWithContext(InstructionList instructions) {
        ...
        for (int i = 0; i < instructions.size() - 1; i++) {
            ...
            if (i == 0 && !Double.isNaN(favoredHeading) && instruction.extraInfo.containsKey(TESTA)) {
                double heading = (double) instruction.extraInfo.get(TESTA);
                double diff = Math.abs(heading - favoredHeading) % 360;
                methodMerger(diff, 170, 190, instruction);
            }
            ...
            if (instruction.getSign() == Instruction.REACHED_VIA) {
                ...
                if (nextInstruction.getSign() != Instruction.CONTINUE_ON_STREET
                        ...
                        || !nextInstruction.extraInfo.containsKey(TESTA)) {
                    //  throw exception?
                    continue;
                }
                double lastHeading = (double) instruction.extraInfo.get("last_heading");
                double heading = (double) nextInstruction.extraInfo.get(TESTA);
                ...
                methodMerger(diff, 179, 181, nextInstruction);
            }
        }
    ...
    private static void methodMerger(double diff, int x, int x1, Instruction instruction) {
        if (diff > x && diff < x1) {
