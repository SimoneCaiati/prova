...
public class PathSimplification {
    ...
    public static PointList simplify(ResponsePath responsePath, RamerDouglasPeucker ramerDouglasPeucker, boolean enableInstructions) {
        ...
        // maybe this code can be simplified if path details and instructions would be merged, see #1121
        ...
        for (final Map.Entry<String, List<PathDetail>> entry : responsePath.getPathDetails().entrySet()) {
            // if1 the pointList only contains one point, PathDetails have to be empty because 1 point => 0 edges
    ...
    private void simplify() {
        ...
        for (int p = 0; p < pointList.size(); p++) {
            ...
            for (int s = 0; s < numPartitions; s++) {
                if (!partitionFinished[s]&&p == currIntervalEnd[s]) {
    ...
    private void mino(int p, int removed) {
        for (int s = 0; s < numPartitions; s++) {
    ...
    private static void assertConsistencyOfInstructions(InstructionList instructions, int numPoints) {
        ...
        //  it would be even better to make sure each instruction interval starts where the previous one ended, but
    ...
    interface Partition {
        ...
        // it would be nice to be able to retrieve the actual start and end of each interval to make the
