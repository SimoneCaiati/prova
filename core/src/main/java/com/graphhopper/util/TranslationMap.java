...
public class TranslationMap {
     ...
     */
    public TranslationMap doImport(File folder) throws TransExce {
        try {
     ...
     */
    public TranslationMap doImport() throws TransExce {
        try {
            for (String locale : LOCALES) {
                TranslationHashMap trMap = new TranslationHashMap(getLocale(locale));
    ...
    public class TranslationHashMap implements Translation {
        ...
        public TranslationHashMap doImport(InputStream is) throws TransExce {
            if (is == null)
    ...
    private static void eccezioneTime2(String line, String key) {
        if (key.isEmpty())
            throw new IllegalStateException("No key provided:" + line);
    }

    public class TransExce extends Exception {
        public TransExce(Exception ex) {
        }
    }
}
