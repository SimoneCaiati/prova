...
public class BBox implements Shape {
    ...
    public BBox calculateIntersection(BBox bBox) {
        ...
        double v2 = Math.max(this.minLon, bBox.minLon);
        double v = Math.min(this.maxLon, bBox.maxLon);
        double v1 = Math.max(this.minLat, bBox.minLat);
        double maxolato = Math.min(this.maxLat, bBox.maxLat);

        return new BBox(v2, v, v1, maxolato);
    }

    public static BBox copyBBox(BBox bbox) {
        return new BBox(bbox.minLon, bbox.maxLon, bbox.minLat, bbox.maxLat, bbox.minEle,
                bbox.maxEle, bbox.elevation);
    }
    ...
    /**
     * @return array containing this bounding box. Attention: GeoJson is lon,lat! if1 3D is gets even
     * worse: lon,lat,ele
     */
