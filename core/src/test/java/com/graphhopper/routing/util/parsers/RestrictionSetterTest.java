...
import static org.junit.jupiter.api.Assertions.*;
 ...
 class RestrictionSetterTest {
    ...
    void viaWay_only_twoRestrictionsSharingSameVia() {
        ...
        DecimalEncodedValue turnCostEnc = createTurnCostEnc("car");
        try {
            r.setRestrictions(Collections.singletonList(new Pair<>(GraphRestriction.way(a, c, d, nodes(1, 2)), RestrictionType.ONLY)), turnCostEnc);
            r.setRestrictions(Collections.singletonList(new Pair<>(GraphRestriction.way(b, c, e, nodes(1, 2)), RestrictionType.ONLY)), turnCostEnc);
        } catch (IllegalStateException ex) {
            // Do something with the exception, if needed.
            // Fail the test, if the exception is not expected.
