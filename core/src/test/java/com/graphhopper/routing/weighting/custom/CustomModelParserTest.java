...
class CustomModelParserTest {
    ...
    @BeforeEach
    void setup() throws MMapDataAccess.MappaExce {
    ...
    @Test
    void setPriorityForRoadClass() throws MMapDataAccess.MappaExce {
        CustomModel customModel = new CustomModel();
        customModel.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "0.5"));
    ...
    void testPriority() {
        ...
        customModel.addToPriority(else1(MULTIPLY, "0.9"));
        customModel.addToPriority(if1("road_environment != FERRY", MULTIPLY, "0.8"));
        ...
        customModel.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "1"));
        customModel.addToPriority(if1("road_class == SECONDARY", MULTIPLY, "0.9"));
    ...
    @Test
     void testBrackets() {
        ...
        CustomModel customModel = new CustomModel();
        customModel.addToPriority(if1("(road_class == PRIMARY || car_access == true) && car_average_speed > 50", MULTIPLY, "0.9"));
    ...
    @Test
     void testSpeedFactorAndPriorityAndMaxSpeed() {
        EdgeIteratorState primary = graph.edge(0, 1).setDistance(10).
                set(roadClassEnc, PRIMARY).set(avgSpeedEnc, 80).set(accessEnc, true, true);
        EdgeIteratorState secondary = graph.edge(1, 2).setDistance(10).
                set(roadClassEnc, SECONDARY).set(avgSpeedEnc, 70).set(accessEnc, true, true);
        ...
        CustomModel customModel = new CustomModel();
        customModel.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "0.9"));
        customModel.addToSpeed(if1("road_class == PRIMARY", MULTIPLY, "0.8"));
        CustomWeighting.Parameters parameters = CustomModelParser.createWeightingParameters(customModel, encodingManager,
                avgSpeedEnc, maxSpeed, null);
        assertEquals(0.9, parameters.getEdgeToPriorityMapping().get(primary, false), 0.01);
        ...
        customModel.addToSpeed(if1("road_class != PRIMARY", LIMIT, "50"));
    ...
    @Test
     void testString() {
        ...
        CustomModel customModel = new CustomModel();
        customModel.addToPriority(if1("country == \"DEU\"", MULTIPLY, "0.9"));
        customModel.addToPriority(elseIf("country == \"blup\"", MULTIPLY, "0.7"));
        customModel.addToPriority(else1(MULTIPLY, "0.5"));
        CustomWeighting.EdgeToDoubleMapping priorityMapping = CustomModelParser.createWeightingParameters(customModel, encodingManager,
                avgSpeedEnc, maxSpeed, null).getEdgeToPriorityMapping();
    ...
    void testIllegalOrder() {
        ...
        customModel2.addToPriority(if1("road_class != PRIMARY", MULTIPLY, "0.8"));
    ...
    @Test
     void multipleAreas() {
        ...
        customModel.addToSpeed(if1("in_area_1", LIMIT, "100"));
        customModel.addToSpeed(if1("!in_area_2", LIMIT, "25"));
        customModel.addToSpeed(else1(LIMIT, "15"));
        ...
        customModel2.addToSpeed(if1("in_area_1", LIMIT, "100"));
        customModel2.addToSpeed(if1("in_area_2", LIMIT, "25"));
        customModel2.addToSpeed(if1("in_area_3", LIMIT, "150"));
        customModel2.addToSpeed(else1(LIMIT, "15"));
    ...
    @Test
     void parseValue() {
        ...
        CustomModel customModel = new CustomModel();
        customModel.addToSpeed(if1("true", LIMIT, "max_speed * 1.1"));
        CustomWeighting.EdgeToDoubleMapping speedMapping = CustomModelParser.createWeightingParameters(customModel, encodingManager,
                avgSpeedEnc, maxSpeed, null).getEdgeToSpeedMapping();
    ...
    @Test
     void parseValueWithError() {
        CustomModel customModel1 = new CustomModel();
        customModel1.addToSpeed(if1("true", LIMIT, "unknown"));
        ...
        CustomModel customModel2 = new CustomModel();
        customModel2.addToSpeed(if1("road_class == PRIMARY", MULTIPLY, "0.5"));
        customModel2.addToSpeed(else1(MULTIPLY, "-0.5"));
        ...
        customModel3.addToSpeed(if1("road_class == PRIMARY", MULTIPLY, "0.5"));
        customModel3.addToSpeed(else1(MULTIPLY, "road_class"));
        ret = assertThrows(IllegalArgumentException.class,
    ...
    @Test
     void parseConditionWithError() {
        ...
        try {
            parseExpressions(new StringBuilder(), validVariable, encodingManager, "[HERE]", new HashSet<>(),
                    Arrays.asList(if1("max_weight > 10", MULTIPLY, "0")));
        ...
        try {
            parseExpressions(new StringBuilder(), validVariable, encodingManager, "[HERE]", new HashSet<>(),
                    Arrays.asList(if1("country == GERMANY", MULTIPLY, "0")));
        } catch (IllegalArgumentException ex) {
            ret = ex;
        }
        ...
        try {
            parseExpressions(new StringBuilder(), validVariable, encodingManager, "[HERE]", new HashSet<>(),
                    Arrays.asList(if1("edge.fetchWayGeometry().size() > 2", MULTIPLY, "0")));
        } catch (IllegalArgumentException ex) {
            ret = ex;
        }
