...
class CustomWeightingTest {
    ...
    @Test
     void withPriority() {
        ...
        model.addToPriority(if1("road_class == SECONDARY", MULTIPLY, "0.5"));
    ...
    @Test
     void testSpeedFactorBooleanEV() {
        ...
        vehicleModel.addToPriority(if1(RoadClassLink.KEY, MULTIPLY, "0.5"));
    ...
    @Test
     void testBoolean() throws MMapDataAccess.MappaExce {
        ...
        vehicleModel.addToPriority(if1("special == true", MULTIPLY, "0.8"));
        vehicleModel.addToPriority(if1("special == false", MULTIPLY, "0.4"));
    ...
    @Test
     void testSpeedFactorAndPriority() {
        ...
        CustomModel vehicleModel = new CustomModel();
        vehicleModel.addToPriority(if1("road_class != PRIMARY", MULTIPLY, "0.5"));
        vehicleModel.addToSpeed(if1("road_class != PRIMARY", MULTIPLY, "0.9"));
        ...
        vehicleModel.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "1.0"));
        vehicleModel.addToPriority(else1(MULTIPLY, "0.5"));
        vehicleModel.addToSpeed(if1("road_class != PRIMARY", MULTIPLY, "0.9"));
        assertEquals(1.15, createWeighting(vehicleModel).calcEdgeWeight(primary, false), 0.01);
        assertEquals(1.84, createWeighting(vehicleModel).calcEdgeWeight(secondary, false), 0.01);
    }
    ...
    @Test
     void testIssueSameKey() {
        ...
        CustomModel vehicleModel = new CustomModel();
        vehicleModel.addToSpeed(if1("toll == HGV || toll == ALL", MULTIPLY, "0.8"));
        vehicleModel.addToSpeed(if1("hazmat != NO", MULTIPLY, "0.8"));
        ...
        vehicleModel = new CustomModel();
        vehicleModel.addToSpeed(if1("bike_network != OTHER", MULTIPLY, "0.8"));
        assertEquals(1.26, createWeighting(vehicleModel).calcEdgeWeight(withToll, false), 0.01);
        assertEquals(1.26, createWeighting(vehicleModel).calcEdgeWeight(noToll, false), 0.01);
    }
    ...
    @Test
     void testFirstMatch() {
        ...
        CustomModel vehicleModel = new CustomModel();
        vehicleModel.addToSpeed(if1("road_class == PRIMARY", MULTIPLY, "0.8"));
        ...
        vehicleModel.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "0.9"));
    ...
    @Test
     void testCarAccess() {
        ...
        CustomModel vehicleModel = new CustomModel();
        vehicleModel.addToPriority(if1("car_average_speed > 40", MULTIPLY, "0.5"));
    ...
    @Test
     void testArea() throws Exception {
        ...
        CustomModel vehicleModel = new CustomModel();
        vehicleModel.addToPriority(if1("in_custom1", MULTIPLY, "0.5"));
    ...
    @Test
     void testMaxSpeed() {
        ...
        assertEquals(1000.0 / 72 * 3.6, createWeighting(new CustomModel().
                addToSpeed(if1("true", LIMIT, "72")).setDistanceInfluence(0)).getMinWeight(1000));
        ...
        assertEquals(1000.0 / 155 * 3.6, createWeighting(new CustomModel().
                addToSpeed(if1("true", LIMIT, "180")).setDistanceInfluence(0)).getMinWeight(1000));

        // reduce speed only a bit
        assertEquals(1000.0 / 150 * 3.6, createWeighting(new CustomModel().
                addToSpeed(if1("road_class == SERVICE", MULTIPLY, "1.5")).
                addToSpeed(if1("true", LIMIT, "150")).setDistanceInfluence(0)).getMinWeight(1000));
    }
    ...
    @Test
     void testMaxPriority() {
        ...
        assertEquals(1000.0 / maxSpeed / 0.5 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("true", MULTIPLY, "0.5")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);

        // ignore too big limit
        assertEquals(1000.0 / maxSpeed / 1.0 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("true", LIMIT, "2.0")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);
        ...
        assertEquals(1000.0 / maxSpeed / 2.0 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("true", MULTIPLY, "3.0")).
                addToPriority(if1("true", LIMIT, "2.0")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);
        assertEquals(1000.0 / maxSpeed / 1.5 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("true", MULTIPLY, "1.5")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);
        ...
        assertEquals(1000.0 / maxSpeed / 3.0 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("road_class == SERVICE", MULTIPLY, "3.0")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);

        // do NOT pick maximum priority when it is for a special case
        assertEquals(1000.0 / maxSpeed / 1.0 * 3.6, createWeighting(new CustomModel().
                addToPriority(if1("road_class == SERVICE", MULTIPLY, "0.5")).setDistanceInfluence(0)).getMinWeight(1000), 1.e-6);
    }

    @Test
     void tooManyStatements() {
        CustomModel customModel = new CustomModel();
        for (int i = 0; i < 1050; i++) {
            customModel.addToPriority(if1("road_class == MOTORWAY || road_class == SECONDARY || road_class == PRIMARY", MULTIPLY, "0.1"));
        }
    ...
    @Test
     void maxSpeedViolated_bug_2307() {
        ...
        CustomModel customModel = new CustomModel()
                .addToSpeed(Statement.if1("road_class == MOTORWAY", Statement.Op.MULTIPLY, "0.7"))
                .addToSpeed(Statement.else1(LIMIT, "30"));
    ...
    @Test
     void bugWithNaNForBarrierEdges() {
        EdgeIteratorState motorway = graph.edge(0, 1).setDistance(0).
                set(roadClassEnc, MOTORWAY).set(avSpeedEnc, 80).set(accessEnc, true, true);
        CustomModel customModel = new CustomModel()
                .addToPriority(Statement.if1("road_class == MOTORWAY", Statement.Op.MULTIPLY, "0"));
        Weighting weighting = createWeighting(customModel);
