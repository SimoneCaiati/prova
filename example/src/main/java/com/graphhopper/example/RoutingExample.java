...
import static com.graphhopper.json.Statement.if1;
import static com.graphhopper.json.Statement.Op.LIMIT;
import static com.graphhopper.json.Statement.Op.MULTIPLY;

public class RoutingExample {
    private static final String BELLAZIA="car_custom";
    public static void main(String[] args) throws ExamExce, TranslationMap.TransExce {
    ...
    public static void routing(GraphHopper hopper) throws ExamExce {
        ...
        // handle errors
        if (rsp.hasErrors())
            throw new ExamExce(rsp.getErrors().toString());
    ...
    public static void speedModeVersusFlexibleMode(GraphHopper hopper) throws ExamExce {
        GHRequest req = new GHRequest(42.508552, 1.532936, 42.507508, 1.528773).
                setProfile("car").setAlgorithm(Parameters.Algorithms.ASTAR_BI).putHint(Parameters.CH.DISABLE, true);
        GHResponse res = hopper.route(req);
        if (res.hasErrors())
            throw new ExamExce(res.getErrors().toString());
        assert Helper.round(res.getBest().getDistance(), -2) == 900;
    }

    public static void headingAndAlternativeRoute(GraphHopper hopper) throws ExamExce {
     ...
     */
    public static void customizableRouting(String ghLoc) throws ExamExce, TranslationMap.TransExce {
        GraphHopper hopper = new GraphHopper();
        hopper.setOSMFile(ghLoc);
        hopper.setGraphHopperLocation("target/routing-custom-graph-cache");
        hopper.setProfiles(new CustomProfile(BELLAZIA).setCustomModel(new CustomModel()).setVehicle("car"));
        ...
        hopper.getLMPreparationHandler().setLMProfiles(new LMProfile(BELLAZIA));
        try {
            hopper.importOrLoad();
        } catch (closefile | MMapDataAccess.MapExce | lockexception e) {
            //
        }

        // ... but for the hybrid mode we can customize the route calculation even at request time:
        // 1. a request with default preferences
        GHRequest req = new GHRequest().setProfile(BELLAZIA).
        ...
        model.addToPriority(if1("road_class == PRIMARY", MULTIPLY, "0.5"));

        // unconditional limit to 100km/h
        model.addToPriority(if1("true", LIMIT, "100"));
    ...
    public static class ExamExce extends Exception {
        public ExamExce(String string) {
        }
    }
}
