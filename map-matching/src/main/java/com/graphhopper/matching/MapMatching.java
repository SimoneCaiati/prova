...
public class MapMatching {
    ...
    public static Router routerFromGraphHopper(GraphHopper graphHopper, PMap hints) {
        ...
        return new Router() {
            ...
            @Override
            public List<Path> calcPaths(QueryGraph queryGraph, int fromNode, int fromOutEdge, int[] toNodes, int[] toInEdges) throws MapExc {
                if (toNodes.length != toInEdges.length) {
                    throw new MapExc("The lengths of toNodes and toInEdges arrays are not equal.");
                }
    ...
    private static void solosolos(GraphHopper graphHopper, PMap hints) {
    ...
    public MatchResult match(List<Observation> observations) throws MapExc {
        ...
        List<Collection<Snap>> snapsPerObservation = filteredObservations.stream()
                .map(o -> findCandidateSnaps(o.getPoint().getLat(), o.getPoint().getLon()))
    ...
    private List<Snap> findCandidateSnapsInBBox(double queryLat, double queryLon, BBox queryShape) {
        ...
        locationIndex.query(queryShape, edgeId -> {
            ...
            if (seenEdges.add(edgeId) && edgeFilter.accept(edge)) {
                ...
                locationIndex.traverseEdge(queryLat, queryLon, edge, (node, normedDist, wayIndex, pos) -> methodMappa2(snap, node, normedDist, wayIndex, pos));
                double dist = DistancePlaneProjection.getInstancePlane().calcDenormalizedDist(snap.getQueryDistance());
                ...
                if (snap.isValid() && (snap.getSnappedPosition() != Snap.Position.TOWER || seenNodes.add(snap.getClosestNode()))) {
                    ...
                    if (queryShape.contains(snap.getSnappedPoint().getLat(), snap.getSnappedPoint().getLon())) {
                        snaps.add(snap);
                    }
                }
            }
    ...
    private static void methodMappa2(Snap snap, int node, double normedDist, int wayIndex, Snap.Position pos) {
        ...
        for (int i = 0; i < filteredObservations.size(); i++) {
            ...
            for (Snap split : splits) {
                if (queryGraph.isVirtualNode(split.getClosestNode())) {
                    ...
                    fallo(virtualEdges, iter);
                    aForz(virtualEdges.size() != 2, "Each virtual node must have exactly 2 "
    ...
    private void aForz(boolean virtualEdges, String s) {
        if (virtualEdges) {
            try {
                throw new MappingExce(s);
            } catch (MappingExce e) {
//
            }
        }
    ...
    private void fallo(List<VirtualEdgeIteratorState> virtualEdges, EdgeIterator iter) {
        while (iter.next()) {
            aForz(!queryGraph.isVirtualEdge(iter.getEdge()), "Virtual nodes must only have virtual edges "
                    + "to adjacent nodes.");
    ...
    private List<SequenceState<State, Observation, Path>> computeViterbiSequence(List<ObservationWithCandidateStates> timeSteps) throws MapExc {
        ...
        alloradue(timeSteps, probabilities, labels, q);
        Label qe = null;
        while (!q.isEmpty()) {
            qe = q.poll();
            ...
            final double linearDistance = distanceCalc.calcDist(timeStep.observation.getPoint().getLat(), timeStep.observation.getPoint().getLon(),
                    nextTimeStep.observation.getPoint().getLat(), nextTimeStep.observation.getPoint().getLon())
            ...
            for (int i = 0; i < nextTimeStep.candidates.size(); i++) {
                ...
                if (path.isFound()) {
                    ...
                    ciao(labels, q, qe, to, minusLogProbability, label1);
                }
            }
        }
        allora(timeSteps, qe);
        ArrayList<SequenceState<State, Observation, Path>> result = new ArrayList<>();
        allorauno(roadPaths, qe, result);
        Collections.reverse(result);
        return result;
    }

    private static void alloradue(List<ObservationWithCandidateStates> timeSteps, HmmProbabilities probabilities, Map<State, Label> labels, PriorityQueue<Label> q) {
    ...
    private static void ciao(Map<State, Label> labels, PriorityQueue<Label> q, Label qe, State to, double minusLogProbability, Label label1) {
        if (label1 == null || minusLogProbability < label1.minusLogProbability) {
            ...
            Label label = marameo(qe, to, minusLogProbability);
            q.add(label);
            labels.put(to, label);
        }
    }

    private static Label marameo(Label qe, State to, double minusLogProbability) {
        Label label = new Label();
        label.state = to;
    ...
    private static void allorauno(Map<Transition<State>, Path> roadPaths, Label qe, ArrayList<SequenceState<State, Observation, Path>> result) {
        while (qe != null) {
    ...
    private static void allora(List<ObservationWithCandidateStates> timeSteps, Label qe) {
        if (qe == null) {
    ...
    private List<EdgeMatch> prepareEdgeMatches(List<SequenceState<State, Observation, Path>> seq) {
        ...
        Filo filo = getfilo(seq, edgeMatches, states, currentDirectedRealEdge);
        serio(edgeMatches, filo.states, filo.currentDirectedRealEdge);
        return edgeMatches;
    }

    private Filo getfilo(List<SequenceState<State, Observation, Path>> seq, List<EdgeMatch> edgeMatches, List<State> states, EdgeIteratorState currentDirectedRealEdge) {
        for (SequenceState<State, Observation, Path> transitionAndState : seq) {
            ...
            if (transitionAndState.transitionDescriptor != null) {
                for (EdgeIteratorState edge : transitionAndState.transitionDescriptor.calcEdges()) {
                    ...
                    states = mero(edgeMatches, states, currentDirectedRealEdge, newDirectedRealEdge);
        ...
        return new Filo(states, currentDirectedRealEdge);
    }

    private static class Filo {
        public final List<State> states;
        public final EdgeIteratorState currentDirectedRealEdge;

        public Filo(List<State> states, EdgeIteratorState currentDirectedRealEdge) {
            this.states = states;
            this.currentDirectedRealEdge = currentDirectedRealEdge;
        }
    }

    private List<State> mero(List<EdgeMatch> edgeMatches, List<State> states, EdgeIteratorState currentDirectedRealEdge, EdgeIteratorState newDirectedRealEdge) {
        if (currentDirectedRealEdge != null) {
            states = seno(edgeMatches, states, currentDirectedRealEdge, newDirectedRealEdge);
        }
        return states;
    }

    private List<State> seno(List<EdgeMatch> edgeMatches, List<State> states, EdgeIteratorState currentDirectedRealEdge, EdgeIteratorState newDirectedRealEdge) {
    ...
    private static void serio(List<EdgeMatch> edgeMatches, List<State> states, EdgeIteratorState currentDirectedRealEdge) {
        if (currentDirectedRealEdge != null) {
            EdgeMatch edgeMatch = new EdgeMatch(currentDirectedRealEdge, states);
            edgeMatches.add(edgeMatch);
        }
    ...
    private double gpxLength(List<Observation> gpxList) {
        if (gpxList.isEmpty()) {
            ...
            for (int i = 1; i < gpxList.size(); i++) {
                ...
                gpxLength += distanceCalc.calcDist(prevEntry.getPoint().getLat(), prevEntry.getPoint().getLon(), entry.getPoint().getLat(), entry.getPoint().getLon());
                prevEntry = entry;
            }
            return gpxLength;
        }
    ...
    public interface Router {
        ...
        List<Path> calcPaths(QueryGraph queryGraph, int fromNode, int fromOutEdge, int[] toNodes, int[] toInEdges) throws MapExc;

        Weighting getWeighting();
    }

    public class MappingExce extends Throwable {
        public MappingExce(String s) {
        }
    }
}