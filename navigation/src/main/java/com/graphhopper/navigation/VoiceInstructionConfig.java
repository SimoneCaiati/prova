...
abstract class VoiceInstructionConfig {
    ...
    public static final String NAVIGA="navigate.";
    protected final TranslationMap translationMap;
    protected final Locale locale;

    protected VoiceInstructionConfig(String translationKey, TranslationMap translationMap, Locale locale) {
...
class ConditionalDistanceVoiceInstructionConfig extends VoiceInstructionConfig {
    ...
    public VoiceInstructionValue getConfigForDistance(double distance, String turnDescription, String thenVoiceInstruction) {
        ...
        String totalDescription = translationMap.getWithFallBack(locale).tr(NAVIGA + translationKey, distanceVoiceValue[instructionIndex]) + " " + turnDescription + thenVoiceInstruction;
...
class FixedDistanceVoiceInstructionConfig extends VoiceInstructionConfig {
    ...
    public VoiceInstructionValue getConfigForDistance(double distance, String turnDescription, String thenVoiceInstruction) {
        if (distance >= distanceAlongGeometry) {
            String totalDescription = translationMap.getWithFallBack(locale).tr(NAVIGA + translationKey, distanceVoiceValue) + " " + turnDescription;
...
class InitialVoiceInstructionConfig extends VoiceInstructionConfig {
    ...
    private final TranslationMap mamt;
    ...
    public VoiceInstructionValue getConfigForDistance(double distance, String turnDescription, String thenVoiceInstruction) {
        if (distance > distanceForInitialStayInstruction) {
            ...
            String continueDescription = mamt.getWithFallBack(locale).tr("continue") + " " +
                    this.mamt.getWithFallBack(locale).tr(NAVIGA + translationKey, distanceVoiceValue);
