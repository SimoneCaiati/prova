...
public final class PtRouterFreeWalkImpl implements PtRouter {
    ...
    private class RequestHandler {
        ...
        RequestHandler(Request request) {
            ...
            if (request.getLimitSolutions() != null) {
                limitSolutions = request.getLimitSolutions();
            } else {
                if (profileQuery) {
                    limitSolutions = 50;
                } else {
                    if (ignoreTransfers) {
                        ...
                        limitSolutions = Integer.MAX_VALUE;
                    }
                }
            }
            ...
            accessSnapFilter = new DefaultSnapFilter(new FastestWeighting(
                ...
                logger.info("accesso al profilo non effettuato");
            ...
            egressSnapFilter = new DefaultSnapFilter(new FastestWeighting(
                ...
                logger.info("profilo richiesto non trovato");
        ...
        private List<List<Label.Transition>> findPaths(Label.NodeId startNode, Label.NodeId destNode) {
            ...
            GraphExplorer graphExplorer = new GraphExplorer(new GraphExplorer.InnerGraph(queryGraph, ptGraph, accessEgressWeighting, gtfsStorage, realtimeFeed), arriveBy, false, false, walkSpeedKmH, false, blockedRouteTypes);
            ...
            for (Label label : router.calcLabels(startNode, initialTime)) {
                ...
                if (visitedNodes >= maxVisitedNodesForRequest || discoveredSolutions.size() >= limitSolutions) {
                    shouldBreak = true;
                }

                if (label.innerlabel.node.equals(destNode)) {
