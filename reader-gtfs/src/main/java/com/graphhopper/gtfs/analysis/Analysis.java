...
public class Analysis {
    ...
    public static List<GtfsStorage.FeedIdWithStopId> getStopsForNode(PtGraph ptGraph, int i) {
        ...
        if (inEdgeTypes.equals(EnumSet.of(GtfsStorage.EdgeType.EXIT_PT)) && outEdgeTypes.equals((EnumSet.of(ENTER_PT)))) {
            ...
            ptGraph.backEdgesAround(i).forEach(e -> stops.add(new GtfsStorage.FeedIdWithStopId(e.getAttrs().getPlatformDescriptor().getFeedId(), e.getAttrs().getPlatformDescriptor().getStopId())));
            ptGraph.edgesAround(i).forEach(e -> stops.add(new GtfsStorage.FeedIdWithStopId(e.getAttrs().getPlatformDescriptor().getFeedId(), e.getAttrs().getPlatformDescriptor().getStopId())));
