...
public class RealtimeIT {
    ...
    @BeforeAll
    public static void init() throws TileBasedElevationProvider.ElevationExce, TranslationMap.TransExce {
    ...
    @Test
     void testHeavyDelayWhereWeShouldTakeOtherTripInstead() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.914944, TO_LON = -116.761472; // NANAA stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
        ...
        assertEquals(LocalDateTime.parse("2007-01-01T07:19:00").atZone(zoneId).toInstant(), bestPtLeg.stops.get(bestPtLeg.stops.size() - 1).innerstop.plannedArrivalTime.toInstant(), "It's better to wait half an hour for the next one (and ride 5 minutes).");
        assertNull(bestPtLeg.stops.get(bestPtLeg.stops.size() - 1).innerstop.predictedArrivalTime, "There is no predicted arrival time.");
        ...
        assertEquals(LocalDateTime.parse("2007-01-01T06:49:00").atZone(zoneId).toInstant(), impossiblePtLeg.stops.get(impossiblePtLeg.stops.size() - 1).innerstop.plannedArrivalTime.toInstant(), "The impossible alternative is my planned 5-minute-trip");
        assertEquals(LocalDateTime.parse("2007-01-01T07:49:00").atZone(zoneId).toInstant(), impossiblePtLeg.stops.get(impossiblePtLeg.stops.size() - 1).innerstop.predictedArrivalTime.toInstant(), "..which is very late today");
    }

    @Test
     void testCanUseDelayedTripWhenIAmLateToo() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.914944, TO_LON = -116.761472; // NANAA stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
    ...
    @Test
     void testExtraTrip() {
        ...
        assertEquals("EXTRA0", ptLeg.internalptleg.tripId);
    }

    @Test
     void testExtraTripWorksOnlyOnSpecifiedDay() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.868446, TO_LON = -116.784582; // BEATTY_AIRPORT stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
    ...
    @Test
     void testZeroDelay() {
        ...
        assertEquals(LocalDateTime.parse("2007-01-01T06:49:00").atZone(zoneId).toInstant(), responseWithRealtimeUpdateBest.stops.get(responseWithRealtimeUpdateBest.stops.size() - 1).innerstop.plannedArrivalTime.toInstant(), "My planned arrival time is correct.");
        assertEquals(LocalDateTime.parse("2007-01-01T06:49:00").atZone(zoneId).toInstant(), responseWithRealtimeUpdateBest.stops.get(responseWithRealtimeUpdateBest.stops.size() - 1).innerstop.predictedArrivalTime.toInstant(), "My expected arrival time is the same.");
        assertNull(responseWithoutRealtimeUpdateBest.stops.get(responseWithoutRealtimeUpdateBest.stops.size() - 1).innerstop.predictedArrivalTime, "The trip without realtime update does not have an expected arrival time.");

//        assertEquals(responseWithoutRealtimeUpdateBest.toString(), responseWithRealtimeUpdateBest.toString());
    }

    @Test
     void testDelayWithoutTransfer() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.914944, TO_LON = -116.761472; // NANAA stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
    ...
    @Test
     void testDelayFromBeginningWithoutTransfer() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.914944, TO_LON = -116.761472; // NANAA stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
        ...
        assertEquals(LocalDateTime.parse("2007-01-01T06:49:00").atZone(zoneId).toInstant(), ptLeg.stops.get(ptLeg.stops.size() - 1).innerstop.plannedArrivalTime.toInstant(), "It is still reporting its original, scheduled time.");
    }
    ...
    @Test
     void testBlockTrips() {
        ...
        // My line does not stop at Bullfrog today. if1 this was a real transfer, I would not be
        ...
        GHResponse response = graphHopperFactory.createWith(feedMessageBuilder.build()).route(ghRequest);
        assertEquals("AB1", (((Trip.PtLeg) response.getBest().getLegs().get(0)).internalptleg.tripId), "I can still use the AB1 trip");
        assertEquals(time(1, 20), response.getBest().getTime(), "It takes");
    }

    @Test
     void testBlockTripSkipsStop() {
        final GtfsRealtime.FeedMessage.Builder feedMessageBuilder = GtfsRealtime.FeedMessage.newBuilder();
        feedMessageBuilder.setHeader(GtfsRealtime.FeedHeader.newBuilder()
                .setGtfsRealtimeVersion("1")
                .setTimestamp(ZonedDateTime.of(LocalDate.of(2007,1,1), LocalTime.of(0,0), zoneId).toEpochSecond()));
        ...
        assertEquals(LocalDateTime.parse("2007-01-01T12:00:00").atZone(zoneId).toInstant(), ptLeg.stops.get(ptLeg.stops.size() - 1).innerstop.plannedArrivalTime.toInstant(), "I have to continue on AB1 which skips my stop, go all the way to the end, and ride back.");
        assertEquals("BFC2", ptLeg.internalptleg.tripId, "Using expected route");
    }

    @Test
     void testMissedTransferBecauseOfDelay() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.868446, TO_LON = -116.784582; // BEATTY_AIRPORT stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
        ...
        assertEquals(300, Duration.between(delayedStop.innerstop.plannedArrivalTime.toInstant(), delayedStop.innerstop.predictedArrivalTime.toInstant()).getSeconds(), "Five minutes late");
    }

    @Test
     void testMissedTransferButExtraTripOnFirstLeg() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.868446, TO_LON = -116.784582; // BEATTY_AIRPORT stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
...
//        assertEquals(2, response.getAll().size());
    ...
    @Test
     void testMissedTransferButExtraTripOnSecondLeg() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.868446, TO_LON = -116.784582; // BEATTY_AIRPORT stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
...
//        assertEquals(2, response.getAll().size());
    ...
    // : Similar case, but where I need a new transfer edge for it to work
    // : Similar case, but where the departure of the second leg is later than all other departures on that day.

    @Test
     void testMissedTransferBecauseOfDelayBackwards() {
        final double FROM_LAT = 36.914893, FROM_LON = -116.76821; // NADAV stop
        final double TO_LAT = 36.868446, TO_LON = -116.784582; // BEATTY_AIRPORT stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
        ...
        assertNull(notDelayedStop.innerstop.predictedArrivalTime, "Not late");

    }

    @Test
     void testDelayAtEndForNonFrequencyBasedTrip() {
        final double FROM_LAT = 36.915682, FROM_LON = -116.751677; // STAGECOACH stop
        final double TO_LAT = 36.88108, TO_LON = -116.81797; // BULLFROG stop
        Request ghRequest = new Request(
                FROM_LAT, FROM_LON,
        ...
        assertEquals("STBA", (((Trip.PtLeg) route.getBest().getLegs().get(0)).internalptleg.tripId), "Using expected route");
        assertEquals("AB1", (((Trip.PtLeg) route.getBest().getLegs().get(1)).internalptleg.tripId), "Using expected route");
        assertEquals(LocalTime.parse("08:15"), LocalTime.from(((Trip.PtLeg) route.getBest().getLegs().get(1)).stops.get(1).innerstop.predictedArrivalTime.toInstant().atZone(zoneId)), "Delay at destination");
