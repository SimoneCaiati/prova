 ...
 class TransfersTest {
    ...
    @Test
     void testTransfersByToRoute() {
        ...
        assertEquals("MUSEUM", transfer.getfromStopId());
        assertEquals("NEXT_TO_MUSEUM", transfer.to_stop_id);
        Assertions.assertNull(transfer.getFromRouteId());
        Assertions.assertNull(transfer.getToRouteId());
        Assertions.assertEquals(600, transfer.getminTransferTime());

        Transfer withinStationTransfer = transfersToStop.get(1);
        assertEquals("NEXT_TO_MUSEUM", withinStationTransfer.getfromStopId());
        assertEquals("NEXT_TO_MUSEUM", withinStationTransfer.to_stop_id);
        assertNull(withinStationTransfer.getFromRouteId());
        assertNull(withinStationTransfer.getToRouteId());
    }

    @Test
     void testInternalTransfersByToRouteIfRouteSpecific() {
        List<Transfer> transfersToStop = sampleFeed.getTransfersToStop("BEATTY_AIRPORT", "AB");
        assertEquals(5, transfersToStop.size());
        assertEquals("AB", transfersToStop.get(0).getFromRouteId());
        assertEquals("FUNNY_BLOCK_AB", transfersToStop.get(1).getFromRouteId());
        assertEquals("STBA", transfersToStop.get(2).getFromRouteId());
        assertEquals("AAMV", transfersToStop.get(3).getFromRouteId());
        assertEquals("ABBFC", transfersToStop.get(4).getFromRouteId());
    }
