     ...
     void wurst() {
        ...
        for (Map.Entry<String, Map<String, Fare>> entry : twoFeeds.entrySet()) {
            for (Fare fare : entry.getValue().values()) {
                Assumptions.assumeTrue(fare.getFareAttribute().getfeedId().equals(entry.getKey()));
            }
        }
        ...
        for (TicketPurchase ticketPurchase : ticketPurchases) {
            for (FareAssignment fareAssignment : ticketPurchase.fareAssignments) {
                assertThat(fareAssignment.segment.feedId, equalTo(fareAssignment.fare.getFareAttribute().getfeedId()));
            }
            assertThat(ticketPurchase.getNSchwarzfahrTrips(), equalTo(0));
            List<Ticket> tickets = ticketPurchase.getTickets();
            assertThat(tickets.stream().map(t -> t.feedId).collect(Collectors.toSet()), equalTo(new HashSet<>(Arrays.asList("only_feed_id","feed_id_2"))));
        }
        assertThat(Fares.cheapestFare(twoFeeds, twoLegsWithDistinctFeeds).get().getSimone(), equalTo(new BigDecimal("3.0")));
    }
