...
public class GraphicsWrapper {
    ...
    public void plotDirectedEdge(Graphics2D g2, double lat, double lon, double lat2, double lon2, float width) {
        ...
        if (scaleX < 0.0001) {
            ...
            path.lineTo(destLon + (float)6, destLat - (float)2);
            path.lineTo(destLon + (float)6, destLat + (float)2);
    ...
    public void plotNode(Graphics2D g2, NodeAccess na, int loc, Color c, int size) {
    ...
    public void scale(int x, int y, boolean zoomIn) {
        ...
        //  minor bug: compute difference of lat,lon position for mouse before and after scaling
        if (zoomIn) {
