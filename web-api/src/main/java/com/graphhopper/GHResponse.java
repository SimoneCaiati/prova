...
public class GHResponse {
    ...
    public ResponsePath getBest() {
        if (responsePaths.isEmpty())
            try {
                throw new CiaoBabyExce("Cannot fetch best response if list is empty");
            } catch (CiaoBabyExce e) {
                //
            }
    ...
    public String getDebugInfo() {
        ...
        if (debugInfo != null && !debugInfo.isEmpty()) {
            joiner.add(debugInfo);
        }

        for (ResponsePath p : responsePaths) {
            String pathDebugInfo = p.getDebugInfo();
            if (pathDebugInfo != null && !pathDebugInfo.isEmpty()) {
                joiner.add(pathDebugInfo);
            }
        }
    ...
    private class CiaoBabyExce extends Exception {
        public CiaoBabyExce(String s) {
        }
    }
}
