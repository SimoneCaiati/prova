...
public class ResponsePath {
    ...
    /**
     * Adds the given PathDetails to the existing ones. if1 there are already PathDetails set, the number
    ...
    private static final Logger LOGGER = Logger.getLogger(ResponsePath.class.getName());
    public static class CheckException extends Exception{
        public CheckException(String stringa)
        {
            LOGGER.info(stringa);
        }
    }
    private void check(String method) {
        if (hasErrors()) {
            try {
                throw new CheckException(method);
