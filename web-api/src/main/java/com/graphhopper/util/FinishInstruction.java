...
super(FINISH, name, createPointList(lat, lon, ele));
}

private static PointList createPointList(double lat, double lon, double ele) {
PointList pointList = new PointList(2, !Double.isNaN(ele));
pointList.add(lat, lon, ele);
return pointList;
}

    public FinishInstruction(final double lat, final double lon, final double ele) {
        super(FINISH, "", createPointList(lat, lon, ele));
    }
