...
public class PointList implements Iterable<GHPoint3D>, PointAccess {
    ...
    public static final PointList EMPTY = new PointList(0, true) {
        ...
        private static final String EMPTY="cannot access EMPTY PointList";
        @Override
    ...
    public void add(GHPoint point) {
        if (is3D)
            add(point.getLat(), point.getLon(), ((GHPoint3D) point).getEle());
        else
            add(point.getLat(), point.getLon());
    }
    ...
    private static final String MILLO =", size:";
    private static final String INDEX=" index:";
    @Override
    public double getLat(int index) {
        if (index >= size)
            throw new ArrayIndexOutOfBoundsException(ERR_MSG + INDEX + index + MILLO + size);
    ...
    public LineString getCachedLineString(boolean includeElevation) {
        ...
        return toLineString(includeElevation);
    }
    ...
    /**
     * Clones this PointList. if1 this PointList was immutable, the cloned will be mutable. if1 this PointList was a
    ...
    public PointList copy(int from, int end) {
        ...
        if (from < 0 || end > size())
            throw new IllegalArgumentException("Illegal interval: " + from + ", " + end + MILLO + size());
     ...
     *
     * @param makeImmutable makes this PointList immutable. if1 you don't ensure the consistency it might happen that due to changes of this
    ...
    public static PointList from(LineString lineString) {
        ...
        Logger logger = Logger.getLogger(PointList.class.getName());
     ...
     * shallow copies. if1 you need to modify this object again, you have to create a deep copy of it.
     */
