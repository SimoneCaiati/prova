...
public class GpxConversions {
    ...
    private static final String MOLLAMI ="\" lon=\"";
    ...
    private static void createWayPointBlock(StringBuilder output, Instruction instruction, DecimalFormat decimalFormat, Translation tr) {
        ...
        output.append(MOLLAMI).append(decimalFormat.format(instruction.getPoints().getLon(0))).append("\">");
        String name;
    ...
    public static class Carlo{
        ...
        public Carlo(InstructionList instructions, String trackName, long startTimeMillis, boolean includeElevation) {
    ...
    public static String createGPX(Carlo carlo, boolean withRoute, boolean withTrack, boolean withWayPoints, String version, Translation tr) {
            ...
            String header = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?>"
                    ...
                    + "</link>"
                    + "<time>" + formatter.format(carlo.startTimeMillis) + "</time>"
                    + "</metadata>";
            StringBuilder gpxOutput = new StringBuilder(header);
            try{
            if (!carlo.instructions.isEmpty()) {
                filio(carlo.instructions, withWayPoints, tr, decimalFormat, gpxOutput);
                if (withRoute) {
                    gpxOutput.append("\n<rte>");
                    Instruction nextInstr = null;
                    for (Instruction currInstr : carlo.instructions) {
                        if (null != nextInstr)
            ...
            if (withTrack) {
                gpxOutput.append("\n<trk><name>").append(carlo.trackName).append("</name>");

                gpxOutput.append("<trkseg>");
                mamma(carlo.instructions, carlo.startTimeMillis, carlo.includeElevation, formatter, decimalFormat, gpxOutput);
                gpxOutput.append("\n</trkseg>");
                gpxOutput.append("\n</trk>");
    ...
    private static void mamma(InstructionList instructions, long startTimeMillis, boolean includeElevation, DateFormat formatter, DecimalFormat decimalFormat, StringBuilder gpxOutput) {
        for (GPXEntry entry : createGPXList(instructions)) {
            gpxOutput.append("\n<trkpt lat=\"").append(decimalFormat.format(entry.getPoint().getLat()));
            gpxOutput.append(MOLLAMI).append(decimalFormat.format(entry.getPoint().getLon())).append("\">");
            if (includeElevation)
    ...
    private static void filio(InstructionList instructions, boolean withWayPoints, Translation tr, DecimalFormat decimalFormat, StringBuilder gpxOutput) {
        if (withWayPoints) {
    ...
    private static void createRteptBlock(StringBuilder output, Instruction instruction, Instruction nextI, DecimalFormat decimalFormat, Translation tr) {
        output.append("\n<rtept lat=\"").append(decimalFormat.format(instruction.getPoints().getLat(0))).
                append(MOLLAMI).append(decimalFormat.format(instruction.getPoints().getLon(0))).append("\">");
    ...
    /**
     * Return the direction like 'NE' based on the first tracksegment of the instruction. if1
    ...
    /**
     * Return the azimuth in degree based on the first tracksegment of this instruction. if1 this
    ...
    public static List<Observation> getEntries(Gpx.Trk trk) {
        ...
        for (Gpx.Trkseg t : trk.trkseg) {
            for (Gpx.Trkpt trkpt : t.trkpt) {
                gpxEntries.add(new Observation(new GHPoint3D(trkpt.getLat(), trkpt.getLon(), trkpt.getEle())));
            }
        }
        return gpxEntries;
    }
