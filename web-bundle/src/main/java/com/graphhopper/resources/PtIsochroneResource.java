...
public class PtIsochroneResource {
    ...
    public static class Response {
        ...
        public List<JsonFeature> getPolygons() {
            return polygons;
        }

        protected List<JsonFeature> polygons = new ArrayList<>();
         Info info = new Info();
    }
    ...
    public Response doGet(
            ...
            @QueryParam("result") @DefaultValue("multipolygon") String format) throws ContourBuilder.CounterExce {
        Instant initialTime = departureTimeParam.get().toInstant();
        GHLocation location = sourceParam.get();
        long num=1000;
        double targetZ = seconds * (double)num;
        ...
        GraphExplorer graphExplorer = new GraphExplorer(new GraphExplorer.InnerGraph(snapResult.queryGraph, gtfsStorage.getPtGraph(), weighting, gtfsStorage, RealtimeFeed.empty()), reverseFlow, false, false, 5.0, reverseFlow, blockedRouteTypes);
        ...
        lellouno(reverseFlow, initialTime, targetZ, snapResult, router, z1, nodeAccess);
        ...
        if (format.equals("multipoint")) {
            ...
            lellodue(siteCoords, constraintVertices);
    ...
    private static void lellotre(Map<Coordinate, Double> z1, QuadEdgeSubdivision tin, Vertex vertex) {
    ...
    private static void lellodue(CoordinateList siteCoords, List<ConstraintVertex> constraintVertices) {
    ...
    private void lellouno(boolean reverseFlow, Instant initialTime, double targetZ, PtLocationSnapper.Result snapResult, MultiCriteriaLabelSetting router, Map<Coordinate, Double> z1, NodeAccess nodeAccess) {
        for (Label label : router.calcLabels(snapResult.nodes.get(0), initialTime)) {
            if (lello(reverseFlow, initialTime, targetZ, z1, nodeAccess, label)) break;
        }
    }

    private boolean lello(boolean reverseFlow, Instant initialTime, double targetZ, Map<Coordinate, Double> z1, NodeAccess nodeAccess, Label label) {
        if ((label.getInnerlabel().currentTime - initialTime.toEpochMilli()) * (reverseFlow ? -1 : 1) > targetZ)
        {
            return true;
        }
        if (label.getInnerlabel().node.getStreetNode() != -1) {
            Coordinate nodeCoordinate = new Coordinate(nodeAccess.getLon(label.getInnerlabel().node.getStreetNode()), nodeAccess.getLat(label.getInnerlabel().node.getStreetNode()));
            z1.merge(nodeCoordinate, (double) (label.getInnerlabel().currentTime - initialTime.toEpochMilli()) * (reverseFlow ? -1 : 1), Math::min);
        } else if (label.getInnerlabel().edge != null && (label.getInnerlabel().edge.getType() == GtfsStorage.EdgeType.EXIT_PT || label.getInnerlabel().edge.getType() == GtfsStorage.EdgeType.ENTER_PT)) {
            GtfsStorage.PlatformDescriptor platformDescriptor = label.getInnerlabel().edge.getPlatformDescriptor();
            Stop stop = gtfsStorage.getGtfsFeeds().get(platformDescriptor.getFeedId()).stops.get(platformDescriptor.getStopId());
            Coordinate nodeCoordinate = new Coordinate(stop.getStopLon(), stop.getStopLat());
            z1.merge(nodeCoordinate, (double) (label.getInnerlabel().currentTime - initialTime.toEpochMilli()) * (reverseFlow ? -1 : 1), Math::min);
        }
        return false;
    }
