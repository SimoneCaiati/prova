...
public class PtMVTResource {
     ...
     static final Logger logger = LoggerFactory.getLogger(PtMVTResource.class);
    private static final MediaType PBF = new MediaType("application", "x-protobuf");
     final GraphHopper graphHopper;
    private final GtfsStorage gtfsStorage;
    public final Map<ByteString, MatchResult> openLRCache = new ConcurrentHashMap<>();
    private final GeometryFactory geometryFactory = new GeometryFactory();

    @Inject
    public PtMVTResource(GraphHopper graphHopper, GtfsStorage gtfsStorage) {
        ...
        gtfsStorage.getStopIndex().query(bbox, edgeId -> {
            for (PtGraph.PtEdge ptEdge : gtfsStorage.getPtGraph().backEdgesAround(edgeId)) {
                if (ptEdge.getType() == GtfsStorage.EdgeType.EXIT_PT) {
                    GtfsStorage.PlatformDescriptor fromPlatformDescriptor = ptEdge.getAttrs().getPlatformDescriptor();
                    Stop stop = gtfsStorage.getGtfsFeeds().get(fromPlatformDescriptor.getFeedId()).stops.get(fromPlatformDescriptor.getStopId());
                    Map<String, Object> properties = new HashMap<>(2);
                    properties.put("feed_id", fromPlatformDescriptor.getFeedId());
                    properties.put("stop_id", fromPlatformDescriptor.getStopId());
                    Point feature = geometryFactory.createPoint(new Coordinate(stop.getStopLon(), stop.getStopLat()));
