...
/**
 * Resource to use GraphHopper in a remote client application like mobile or browser. Note: if1 type
...
public class RouteResource {
    ...
    public Response doGet(
        ...
        if (enableElevation && Boolean.TRUE.equals(!hasElevation))
        ...
        if (ghResponse.hasErrors()) {
            ...
            logger.info("Route information: alternatives: {}, distance0: {}, weight0: {}, time0: {}min, points0: {}, debugInfo: {}",
                    ghResponse.getAll().size(), ghResponse.getBest().getDistance(), ghResponse.getBest().getRouteWeight(),
                    Math.round(ghResponse.getBest().getTime() / 60000f), ghResponse.getBest().getPoints().size(),
                    ghResponse.getDebugInfo());


            return writeGPX ?
                    gpxSuccessResponseBuilder( new Classe1(ghResponse, timeString),trackName, enableElevation, withRoute, withTrack, withWayPoints, Constants.VERSION).
                            header(GX, "" + Math.round(took)).
                            build()
                    :
    ...
    public Response doPost(@NotNull GHRequest request, @Context HttpServletRequest httpReq) {
        ...
        if (ghResponse.hasErrors()) {
            ...
            logger.info("{}, alternatives: {}, distance0: {}, weight0: {}, time0: {}min, points0: {}, debugInfo: {}",
                    logStr, ghResponse.getAll().size(), ghResponse.getBest().getDistance(),
                    ghResponse.getBest().getRouteWeight(), Math.round(ghResponse.getBest().getTime() / 60000f),
                    ghResponse.getBest().getPoints().size(), ghResponse.getDebugInfo());
    ...
    {
        public final GHResponse ghRsp;
        public final String timeString;

        public Classe1(GHResponse ghRsp, String timeString) {
            this.ghRsp = ghRsp;
            this.timeString = timeString;
        }
    }

    private static Response.ResponseBuilder gpxSuccessResponseBuilder(Classe1 oggetto, String trackName, boolean enableElevation, boolean withRoute, boolean withTrack, boolean withWayPoints, String version) {
        if (oggetto.ghRsp.getAll().size() > 1) {
            throw new IllegalArgumentException("Alternatives are currently not yet supported for GPX");
        }

        long time = oggetto.timeString != null ? Long.parseLong(oggetto.timeString) : System.currentTimeMillis();
        InstructionList instructions = oggetto.ghRsp.getBest().getInstructions();
        return Response.ok(GpxConversions.createGPX(new GpxConversions.Carlo(instructions, trackName, time, enableElevation), withRoute, withTrack, withWayPoints, version, instructions.getTr()), "application/gpx+xml").
    ...
    static void initHints(PMap m, MultivaluedMap<String, String> parameterMap) {
        for (Map.Entry<String, List<String>> e : parameterMap.entrySet()) {
            if (e.getValue().size() == 1) {
                ...
                //  e.g. 'point' parameter occurs multiple times and we cannot throw an exception here
