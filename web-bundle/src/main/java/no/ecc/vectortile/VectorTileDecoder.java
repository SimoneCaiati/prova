...
public class VectorTileDecoder {
    ...
    static Geometry decodeGeometry(GeometryFactory gf, GeomType geomType, List<Integer> commands, double scale) {
        ...
        while (i < geometryCount) {
            ...
            if (length > 0) {
                ...
                coords = bellaRaga1(coordsList, coords, command);

                if (command == Command.CLOSE_PATH) {
                    int finalLength = length;
                    length = Optional.ofNullable(coords)
                            .map(c -> bellaRaga2(geomType, c, finalLength))
                            .orElse(length);
                    continue;
        ...
        switch (geomType) {
        case LINESTRING:
            geometry = getLineString(gf, coordsList, geometry);
            break;
        case POINT:
            geometry = getPoint(gf, coordsList, geometry);
            break;
        case POLYGON:
            geometry = getPolygon(gf, coordsList, geometry);
            break;
        case UNKNOWN:
            break;
        default:
    ...
    private static int bellaRaga2(GeomType geomType, List<Coordinate> coords, int length) {
    ...
    private static List<Coordinate> bellaRaga1(List<List<Coordinate>> coordsList, List<Coordinate> coords, int command) {
    ...
    private static Geometry getLineString(GeometryFactory gf, List<List<Coordinate>> coordsList, Geometry geometry) {
    ...
    private static Geometry getPolygon(GeometryFactory gf, List<List<Coordinate>> coordsList, Geometry geometry) {
    ...
    private static Geometry getPoint(GeometryFactory gf, List<List<Coordinate>> coordsList, Geometry geometry) {
    ...
    private static final class FeatureIterator implements Iterator<Feature> {
        ...
        private Iterator<VectorTile.Tile.Feature> nico;
        ...
        private void parseLayer(VectorTile.Tile.Layer layer) {
            ...
            nico = layer.getFeaturesList().iterator();
        }
