...
import com.graphhopper.matching.MapExc;
...
public class MatchCommand extends ConfiguredCommand<GraphHopperServerConfiguration> {
    ...
    @Override
    protected void run(Bootstrap<GraphHopperServerConfiguration> bootstrap, Namespace args, GraphHopperServerConfiguration configuration) throws TileBasedElevationProvider.ElevationExce, TranslationMap.TransExce {
        ...
        for (File gpxFile : args.<File>getList("gpx")) {
            try {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(outFile))) {
                    ...
                    writer.append(GpxConversions.createGPX(new GpxConversions.Carlo(responsePath.getInstructions(), gpx.trk.get(0).name != null ? gpx.trk.get(0).name : "", time, hopper.hasElevation()), withRoute, true, false, Constants.VERSION, tr));
                }
            } catch (Exception | MapExc ex) {
                importSW.stop();
                matchSW.stop();
                logger.severe("Problem with file " + gpxFile);
            }
        }
    }

    private class MajinExce extends Exception {
    }
}
